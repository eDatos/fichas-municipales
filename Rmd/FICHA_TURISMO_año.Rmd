...
df_u <- cbind(data.frame(do.call('rbind', piramide[["data"]][["dimCodes"]])), piramide[["data"]][["Valor"]])
colnames(df_u) <- c('gredad', 'sexo', 'pais', 'mun', 'año', 'Valor')
df_u$gredad <- ordered(df_u$gredad, levels=piramide[["categories"]][["codes"]][[1]], labels=piramide[["categories"]][["labels"]][[1]])
df_u$sexo <- factor(df_u$sexo, levels=piramide[["categories"]][["codes"]][[2]], labels=piramide[["categories"]][["labels"]][[2]])
df_u$pais <- factor(df_u$pais, levels=piramide[["categories"]][["codes"]][[3]], labels=piramide[["categories"]][["labels"]][[3]])
# df_u$mun <- factor(df_u$mun, levels=piramide[["categories"]][["codes"]][[4]], labels=piramide[["categories"]][["labels"]][[4]])
df_u$año <- as.integer(df_u$año)
df_u$Valor <- as.numeric(df_u$Valor)
# df_u$mun <- recode_mun(df_u, 'mun')

df_u <- df_u %>% filter(gredad == "TOTAL GRUPOS DE EDADES" & sexo == "AMBOS SEXOS" & pais == "TOTAL" & substring(mun, 6,7) == "_3") %>%
...
leyenda.template <- "<div class='serie-placeholder serie-placeholder-%s'><div class='sp-bullet' style='background-color: %s'></div><div class='sp-content'><span class='sp-municipio'>%s</span><br/>Turistas: <span class='sp-habitantes'>%s</span><br/>Tasa de crecimiento: <span class='sp-tasa'>%s&percnt;</span></div></div>"
leyenda.content <- "<span class='sp-municipio'>%s</span><br/>Turistas: <span class='sp-habitantes'>%s</span><br/>Tasa de crecimiento: <span class='sp-tasa'>%s&percnt;</span>"
...
df <- df %>%
  mutate(
    tooltip = ifelse(año == params$año, 
                     ...
                     sprintf(leyenda.content, municipio, paste0(format(Valor, big.mark = ".", decimal.mark = ","), ' (',año,')'), format(Tasa, big.mark = ".", decimal.mark = ","))
              )
  )

df <- df %>%
  mutate(Tasa = ifelse(año == min(año), "-", Tasa)) %>%
  mutate(
    tooltip = ifelse(año == min(año), 
                     sprintf(leyenda.content, municipio, paste0(format(Valor, big.mark = ".", decimal.mark = ","), ' (',año,')'), "-"),
                     tooltip
              )
  )
...
g_line <- ggplot(data = df, aes(x = año, y = Valor, group = municipio, colour = municipio, text = tooltip)) +
  ...
  scale_y_continuous(labels = function(x){paste0(format(x/10^3, big.mark = ".", decimal.mark = ","), " mil")}, n.breaks = 5)
...
Variacion_Turistas <- abs((ind_turistas %>% filter(municipio == municipio_actual$municipio & año == params$año) %>% select(Valor) %>% as.numeric()) -
                      (ind_turistas %>% filter(municipio == municipio_actual$municipio & año == params$año-1) %>% select(Valor) %>% as.numeric())) %>%
                      format(big.mark = ".", decimal.mark = ",")
Signo_Variacion <- signo((ind_turistas %>% filter(municipio == municipio_actual$municipio & año == params$año) %>% select(Valor) %>% as.numeric()) -
                         (ind_turistas %>% filter(municipio == municipio_actual$municipio & año == params$año-1) %>% select(Valor) %>% as.numeric()))
...
df_tur_comarca <- df_ranking %>% filter(comarca == df_tur_mun$comarca)
rk_comarca <- grep(params$id_municipio, df_tur_comarca$id)
num_mun_comarca <- nrow(df_tur_comarca)
percent_comarca <- format(round(df_tur_mun$Valor / sum(df_tur_comarca$Valor, na.rm = TRUE) * 100, 1), big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
df_turismo_gedad <- df_turismo_gedad %>% mutate(porcentaje = df_turismo_gedad$valor/ sum(df_turismo_gedad$valor) * 100)

Turistas_menos44 <- df_turismo_gedad$valor[which(df_turismo_gedad$gredad == "De 16 a 44 años")] %>% format(big.mark = ".", decimal.mark = ",")
Turistas_mas44 <- df_turismo_gedad$valor[which(df_turismo_gedad$gredad == "Mayor de 44 años")] %>% format(big.mark = ".", decimal.mark = ",")
...
Percent_Mujeres <- round((df_piramide %>%
                            filter(mun == municipio_actual$municipio & gredad == "TOTAL GRUPOS DE EDADES" & sexo == "Mujeres" & pais == "TOTAL" & año == params$año) %>%
                            subset(select = "valor") %>% as.numeric) / Num_PTur_num *100, 1) %>% format(big.mark=".",decimal.mark = ",", nsmall = 1)
Percent_Hombres <- round((df_piramide %>%
...
Num_Mujeres <- df_piramide %>%
  filter(mun == municipio_actual$municipio & gredad == "TOTAL GRUPOS DE EDADES" & sexo == "Mujeres" & pais == "TOTAL" & año == params$año) %>% subset(select = "valor") %>%
  as.numeric %>% format(big.mark=".",decimal.mark = ",")
Num_Hombres <- df_piramide %>%
  filter(mun == municipio_actual$municipio & gredad == "TOTAL GRUPOS DE EDADES" & sexo == "Hombres" & pais == "TOTAL" & año == params$año) %>% subset(select = "valor") %>% 
  as.numeric %>% format(big.mark=".",decimal.mark = ",")
```

```{r Barras países por sexo}
# Turistas según grupos de edades y sexos por países de residencia. Municipios turísticos de Canarias y periodos. 2018 a 2020.
# turismo_pais <- fromJSON(params$url.piramide)
...
rk_turismo_pais_mun <- df_turismo_pais %>%
  ...
  select(c(pais, valor))
rk_turismo_pais_mun <- rk_turismo_pais_mun %>% mutate(porcentaje = rk_turismo_pais_mun$valor/ sum(rk_turismo_pais_mun$valor) * 100)
...
df_turismo_pais_mun <- df_turismo_pais_mun %>%
  mutate(porcentaje = df_turismo_pais_mun$valor / sum(df_turismo_pais_mun$valor) * 100,
         valor_Canarias = df_turismo_pais_Canarias$valor, porcentaje_Canarias = df_turismo_pais_Canarias$valor / sum(df_turismo_pais_Canarias$valor) * 100)
...
g_turismo_pais <- ggplot(df_turismo_pais_mun, aes(y = pais, fill = sexo, group = sexo, text = hovertext)) +
  geom_bar(stat = "identity", width = 0.8, alpha = 0.8, position = position_stack(reverse = TRUE), aes(x = porcentaje)) +
  # geom_bar(stat = "identity", color = "#59656E", alpha = 0, size = 0.4, position = position_stack(reverse = TRUE), aes(x = porcentaje_Canarias, text = "")) +
...
df_turismo_aloj_mun <- df_turismo_aloj_mun %>%
  mutate(porcentaje = df_turismo_aloj_mun$valor / sum(df_turismo_aloj_mun$valor) * 100,
         valor_Canarias = df_turismo_aloj_C$valor,
         porcentaje_Canarias = df_turismo_aloj_C$valor / sum(df_turismo_aloj_C$valor) * 100)
...
# df_turismo_aloj_plot$tipo <- as.character(df_turismo_aloj_plot$tipo)
...
g_turismo_aloj <- ggplot(df_turismo_aloj_plot, aes(y = tipo, fill = tipo, label = label, name = NULL, text = hovertext)) +
  geom_bar(stat = "identity", alpha = 0.9, width = 0.4, aes(x = porcentaje)) +
...
df_turismo_ingresos <- df_turismo_ingresos %>% mutate(porcentaje = df_turismo_ingresos$valor/ sum(df_turismo_ingresos$valor) * 100)

Turistas_menos50 <- df_turismo_ingresos$valor[which(df_turismo_ingresos$nivel == "Menos de 50.000€")] %>% format(big.mark = ".", decimal.mark = ",")
Turistas_mas50 <- df_turismo_ingresos$valor[which(df_turismo_ingresos$nivel == "50.000€ o más")] %>% format(big.mark = ".", decimal.mark = ",")
Percent_Turistas_menos50 <- df_turismo_ingresos$porcentaje[which(df_turismo_ingresos$nivel == "Menos de 50.000€")] %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
Percent_Turistas_mas50 <- (100 - df_turismo_ingresos$porcentaje[which(df_turismo_ingresos$nivel == "Menos de 50.000€")] %>% round(1)) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
df_turismo_proposito <- df_turismo_proposito %>% mutate(porcentaje = df_turismo_proposito$valor/ sum(df_turismo_proposito$valor) * 100)

# Num_Vacaciones <- df_turismo_proposito$valor[which(df_turismo_proposito$proposito == "Vacaciones")] %>% format(big.mark = ".", decimal.mark = ",")
# Num_Otros_P <- df_turismo_proposito$valor[which(df_turismo_proposito$proposito == "Otros motivos")] %>% format(big.mark = ".", decimal.mark = ",")
...
# Percent_Otros_P <- df_turismo_proposito$porcentaje[which(df_turismo_proposito$proposito == "Otros motivos")] %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)

porc_Vacaciones <- df_turismo_proposito$porcentaje[which(df_turismo_proposito$proposito == "Vacaciones")]
porc_Otros_P <- df_turismo_proposito$porcentaje[which(df_turismo_proposito$proposito == "Otros motivos")]
porc_Vacaciones_l <- porc_Vacaciones %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
porc_Otros_P_l <- (100 - porc_Vacaciones) %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
df_turismo_visitas <- df_turismo_visitas %>%
  filter(N_visitas != "TOTAL" & tipo == "TOTAL" & mun == municipio_actual$municipio & periodo == params$año) %>% 
  select(c(N_visitas, valor))
df_turismo_visitas <- df_turismo_visitas %>% mutate(porcentaje = df_turismo_visitas$valor/ sum(df_turismo_visitas$valor) * 100)

Num_Visita_0 <- df_turismo_visitas$valor[which(df_turismo_visitas$N_visitas == "Ninguna")] %>% format(big.mark = ".", decimal.mark = ",")
Num_Visita_1_2 <- df_turismo_visitas$valor[which(df_turismo_visitas$N_visitas == "Una o dos visitas")] %>% format(big.mark = ".", decimal.mark = ",")
Num_Visita_3 <- df_turismo_visitas$valor[which(df_turismo_visitas$N_visitas == "Tres o más visitas")] %>% format(big.mark = ".", decimal.mark = ",")
Percent_Visita_0 <- df_turismo_visitas$porcentaje[which(df_turismo_visitas$N_visitas == "Ninguna")] %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
Percent_Visita_1_2 <- df_turismo_visitas$porcentaje[which(df_turismo_visitas$N_visitas == "Una o dos visitas")] %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
Percent_Visita_3 <- (100 - df_turismo_visitas$porcentaje[which(df_turismo_visitas$N_visitas == "Ninguna")] %>% round(1) - df_turismo_visitas$porcentaje[which(df_turismo_visitas$N_visitas == "Una o dos visitas")] %>% round(1)) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
<h2 style="color: #666; margin: 0;">Indicadores turísticos anuales</h2>
<hr>
...
<div class="row">
<div class="column-4">`r get_indicator2(paste0(Variacion_Turistas, " ", Signo_Variacion, "</br>que en ", params$año-1), paste0(Variacion_Porcentual_Turistas, "%"), Imagen_Variacion)`</div>
<div class="column-4">`r get_indicator2("Estancia<br>media", Num_estancia_media,"./img/family.png")`</div>
<div class="column-4">`r get_indicator2(paste0("Mujeres</p><p class='label' style='color: #999;'>", Num_Mujeres), paste0(Percent_Mujeres, "%"), "./img/female.png")`</div>
<div class="column-4">`r get_indicator2(paste0("Hombres</p><p class='label' style='color: #999;'>", Num_Hombres), paste0(Percent_Hombres, "%"), "./img/male.png")`</div>
</div>
...
<div class="progressbar-legend-container">
  <!-- <div class='progressbar-legend'><div class='progress-bar-1'></div><div class='sp-content'>Hoteleros</div></div> -->
  <!-- <div class='progressbar-legend'><div class='progress-bar-2'></div><div class='sp-content'>Extrahoteleros</div></div> -->
  <!-- <div class='progressbar-legend'><div class='progress-bar-7'></div><div class='sp-content'>Vivienda privada</div></div> -->
  <!-- <div class='progressbar-legend'><div class='progress-bar-3'></div><div class='sp-content'>Otros</div></div> -->
<div class='progressbar-legend'>
<div class='progress-bar-8' style="padding-left: 6px; margin-top: 0; color: #59656E; font-weight: 600;">|</div><div class='sp-content'>Canarias</div></div>
</div>
</div>
...
<div class="row">
<div class="column highlight">
  <div class="column-2">`r get_indicator3("De 16 a 44 años", paste0(Percent_Turistas_menos44, "%"), "./img/balanza.png")`</div>
  <div class="column-2">`r get_indicator3("Mayor de 44 años", paste0(Percent_Turistas_mas44, "%"), "./img/comercio.png")`</div>
</div>

<div class="column highlight">
  <div class="column-2">`r get_indicator3("Menos de 50.000€", paste0(Percent_Turistas_menos50, "%"), "./img/balanza.png")`</div>
  <div class="column-2">`r get_indicator3("50.000€ o más", paste0(Percent_Turistas_mas50, "%"), "./img/comercio.png")`</div>
</div>
...
<div class="row">
<div class="column highlight">
  <div class="column-3">`r get_indicator3("Ninguna", paste0(Percent_Visita_0, "%"),"./img/primarios.png")`</div>
  <div class="column-3">`r get_indicator3("Una o dos<br>visitas", paste0(Percent_Visita_1_2, "%"),"./img/secundarios.png")`</div>
  <div class="column-3">`r get_indicator3("Tres o más<br>visitas", paste0(Percent_Visita_3, "%"),"./img/postsecundario.png")`</div>
</div>

<div class="column highlight">
  <div class="progress-bar-container">
  <div class="progress-bar-1" style="width:`r porc_Vacaciones`%"></div>
  <div class="progress-bar-3" style="width:`r porc_Otros_P`%"></div>
</div>

<div class="progressbar-legend-container">
  <div class='progressbar-legend'><div class='progress-bar-1'></div><div class='sp-content'>Vacaciones<br/>`r porc_Vacaciones_l`%</div></div>
  <div class='progressbar-legend'><div class='progress-bar-3'></div><div class='sp-content'>Otros propósitos<br/>`r porc_Otros_P_l`%</div></div>
</div>
</div>

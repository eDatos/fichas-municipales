...
# df_u_$mun <- factor(df_u_$mun, levels=piramide[["categories"]][["codes"]][[4]], labels=piramide[["categories"]][["labels"]][[4]])
df_u_$año <- as.integer(df_u_$año)
df_u_$Valor <- as.numeric(df_u_$Valor)
# df_u_$mun <- recode_mun(df_u_, 'mun')
...
# df_turismo_pais_mun$pais <- ordered(df_turismo_pais_mun$pais, levels = df_turismo_pais_mun$pais[4:1])
...
# Percent_Turistas_menos50_C <- df_turismo_ingresos_C$porcentaje[which(df_turismo_ingresos_C$nivel == "Menos de 50.000€")] %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
# Percent_Turistas_mas50_C <- (100 - df_turismo_ingresos_C$porcentaje[which(df_turismo_ingresos_C$nivel == "Menos de 50.000€")] %>% round(1)) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
...
# df_turismo_proposito_C <- df_turismo_proposito %>%
#   filter(proposito != "TOTAL" & tipo == "TOTAL" & mun == "CANARIAS" & periodo == params$año) %>% 
#   select(c(proposito, valor))
# df_turismo_proposito_C <- df_turismo_proposito_C %>% mutate(porcentaje = df_turismo_proposito_C$valor/ num_Canarias_total * 100)
# 
# Percent_Vacaciones_C <- df_turismo_proposito_C$porcentaje[which(df_turismo_proposito_C$proposito == "Vacaciones")] %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
...
df_turismo_visitas_plot <- df_turismo_visitas_mun %>%
  mutate(label = "|")
# df_turismo_visitas_plot$N_visitas <- ordered(df_turismo_visitas_plot$N_visitas, levels = df_turismo_visitas_plot$N_visitas[4:1])
# df_turismo_visitas_plot$N_visitas <- as.character(df_turismo_visitas_plot$N_visitas)
...
if(Variacion_Porcentual_Turistas == "NA"){
  cat(paste0(
    "<div class='column-3'>", get_indicator2('Estancia</br>media', Num_estancia_media, './img/cama2.png'), "</div><div class='column-3'>", get_indicator2(paste0('Mujeres</p><p class="label" style="color: #999;">', Num_Mujeres), paste0(Percent_Mujeres, '%'), './img/female.png'), "</div><div class='column-3'>", get_indicator2(paste0('Hombres</p><p class="label" style="color: #999;">', Num_Hombres), paste0(Percent_Hombres, '%'), './img/male.png'), "</div>"))
}else{
  cat(paste0(

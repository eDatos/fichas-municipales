...
df_u_ <- cbind(data.frame(do.call('rbind', turistas[["data"]][["dimCodes"]])), turistas[["data"]][["Valor"]])
colnames(df_u_) <- c('gredad', 'sexo', 'pais', 'mun', 'año', 'valor')
df_u_$gredad <- ordered(df_u_$gredad, levels=turistas[["categories"]][["codes"]][[1]], labels=turistas[["categories"]][["labels"]][[1]])
df_u_$sexo <- factor(df_u_$sexo, levels=turistas[["categories"]][["codes"]][[2]], labels=turistas[["categories"]][["labels"]][[2]])
df_u_$pais <- factor(df_u_$pais, levels=turistas[["categories"]][["codes"]][[3]], labels=turistas[["categories"]][["labels"]][[3]])
...
Imagen_Variacion <- ifelse(is.na(ind_turistas %>% filter(id == params$id_municipio & año == params$año-1) %>% select(valor) %>% as.numeric()),
                           "NA",
                           ifelse(signo(ind_turistas %>% filter(id == params$id_municipio & año == params$año) %>% select(tasa) %>% as.numeric()) == "más",
                                  './img/up.png', './img/down.png')
                           )
Variacion_Porcentual_Turistas <- ind_turistas %>% filter(id == params$id_municipio & año == params$año) %>% select(tasa) %>% as.numeric() %>%
...
# Turistas según grupos de edades y sexos por países de residencia. Municipios turísticos de Canarias y periodos. 2018 a 2020.
df_turismo_pais <- cbind(data.frame(do.call('rbind', turistas[["data"]][["dimCodes"]])), turistas[["data"]][["Valor"]])
colnames(df_turismo_pais) <- c('ge', 'sexo', 'pais', 'mun', 'periodo', 'valor')
df_turismo_pais$ge <- factor(df_turismo_pais$ge, levels=turistas[["categories"]][["codes"]][[1]], labels=turistas[["categories"]][["labels"]][[1]])
df_turismo_pais$sexo <- factor(df_turismo_pais$sexo, levels=turistas[["categories"]][["codes"]][[2]], labels=turistas[["categories"]][["labels"]][[2]])
df_turismo_pais$pais <- factor(df_turismo_pais$pais, levels=turistas[["categories"]][["codes"]][[3]], labels=turistas[["categories"]][["labels"]][[3]])
df_turismo_pais$mun <- factor(df_turismo_pais$mun, levels=turistas[["categories"]][["codes"]][[4]], labels=turistas[["categories"]][["labels"]][[4]])

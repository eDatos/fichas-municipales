...
df_p_H_ <- data.frame(mun = rep(names(ind_paro_H[["dimension"]][["GEOGRAPHICAL"]][["representation"]][["index"]]),
...
df_p_H <- df_p_H_ %>% filter(code == mes_actual$code & mun == params$id_municipio) %>%
  left_join(mes, by = "code") %>%
  select(!code)
...
df_p_M_ <- data.frame(mun = rep(names(ind_paro_M[["dimension"]][["GEOGRAPHICAL"]][["representation"]][["index"]]),
...
df_p_M <- df_p_M_ %>% filter(code == mes_actual$code & mun == params$id_municipio) %>%
  left_join(mes, by = "code") %>%
  select(!code)
...
colnames(df_paro_se) <- c('mun', 'periodo', 'sector_', 'sexo', 'valor')
...
df_paro_se$sector_ <- factor(df_paro_se$sector_, levels=data_paro_se[["categories"]][["codes"]][[3]], labels=data_paro_se[["categories"]][["labels"]][[3]])
...
df_paro_se <- df_paro_se %>% mutate(sector = recode(sector_, "Sin actividad económica" = "Sin actividad<br>económica"),
                                    sector = recode(sector_, "Resto de servicios" = "Resto de<br>servicios"))

df_paro_se_mun <- df_paro_se %>%
  filter(mun == municipio_actual$municipio & periodo == mes_actual$periodo & sector != "TOTAL" & sexo != "AMBOS SEXOS") %>% select(!mun)
df_paro_Canarias <- df_paro_se %>%
  filter(mun == "CANARIAS" & periodo == mes_actual$periodo & sector != "TOTAL" & sexo != "AMBOS SEXOS") %>% 
  select(!mun)
...
df_paro_se_mun <- df_paro_se_mun %>% transform(hovertext = paste0(
    ifelse(sexo == "Hombres", paste0("<b>", trimws(sector_), "</b><br>"), ""),
    "<br><b>", sexo, "</b>",
...
g_paro_se <- ggplot(data = df_paro_se_mun, aes(x = reorder(sector, order), fill = sexo, group = sexo, text = hovertext)) +

...
leyenda.template <- "<div class='serie-placeholder serie-placeholder-%s'><div class='sp-bullet' style='background-color: %s'></div><div class='sp-content'><span class='sp-municipio'>%s</span><br/>Personas en paro: <span class='sp-habitantes'>%s</span><br/>Tasa de crecimiento: <span class='sp-tasa'>%s&percnt;</span></div></div>"
leyenda.content <- "<span class='sp-municipio'>%s</span><br/>Personas en paro: <span class='sp-habitantes'>%s</span><br/>Tasa de crecimiento: <span class='sp-tasa'>%s&percnt;</span>"
...
df.line <- related_mun %>%
  ...
  #mutate(Tasa =  round(100*((Valor / lag(Valor, n=12)) - 1), 1)) %>%
...
Num_Paro_num <- df_p %>% filter(Ind == "ABSOLUTE") %>% select(Valor) %>% round(1) %>% as.numeric
Num_Paro <- format(Num_Paro_num, big.mark = ".", decimal.mark = ",")
Num_Paro_Canarias <- (df_p_ %>% filter(code == mes_actual$code & id_municipio == "ES70") %>%
  filter(Ind == "ABSOLUTE"))$Valor

# Variacion_Paro <- df_p %>% filter(Ind == "ANNUAL_PUNTUAL_RATE") %>% select(Valor) %>% abs() %>% format(big.mark = ".", decimal.mark = ",") %>% as.character()
# Signo_Variacion <- signo(df_p %>% filter(Ind == "ANNUAL_PUNTUAL_RATE") %>% select(Valor))
...
percent_canarias <- format(round(df_Parados_ranking[which(df_Parados_ranking$id==params$id_municipio),]$valor / sum(df_Parados_ranking$valor) * 100, 1),
...
percent_isla <- format(round(rk_Paro_isla[which(rk_Paro_isla$id==params$id_municipio),]$valor / sum(rk_Paro_isla$valor) * 100, 1), big.mark=".",decimal.mark = ",", nsmall = 1)
...
Num_Pobl_num <- df_piramide %>% filter(mun == municipio_actual$municipio & periodo == mes_actual$Periodo & gredad == "TOTAL" & sexo == "AMBOS SEXOS") %>%
                             subset(select = "valor") %>%  as.numeric()
Num_Pobl_Canarias <- df_piramide %>% filter(mun == "CANARIAS" & periodo == mes_actual$Periodo & gredad == "TOTAL" & sexo == "AMBOS SEXOS") %>%
                             subset(select = "valor") %>%  as.numeric()
...
df_paro_se_mun <- df_paro_se_mun %>%
  mutate(porcentaje = df_paro_se_mun$valor / sum(df_paro_se_mun$valor) * 100,
         valor_Canarias = df_paro_Canarias$valor,
         porcentaje_Canarias = df_paro_Canarias$valor / sum(df_paro_Canarias$valor) * 100) %>% 
...
df_paro_sexo_est_mun_ <- df_paro_sexo_est_mun_ %>% mutate(peso = round(df_paro_sexo_est_mun_$valor / sum(df_paro_sexo_est_mun_$valor) * 100, 1))
...
<div class="row highlight">
  <div class ="column-2">`r get_indicator_met("Tasa de ", "Incremento o disminución en el número de individuos pertenecientes a un determinado ámbito en un periodo concreto por cada 1.000 habitantes.","./img/crecimiento.png")`</div>

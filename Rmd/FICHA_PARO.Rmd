...
df_u_ <- df_u %>% spread(medida, valor) %>% select(mun, mes, valor = ABSOLUTE, tasa = ANNUAL_PERCENTAGE_RATE)
...
g_line <- ggplotly(g_line, tooltip = "text") %>%
  ...
  layout(hoverlabel = "", hovermode = 'x unified',
         xaxis = list(autorange = FALSE,
                      range = c(max(max(df$order) - 5*12 - 12, min(df$order) - 12), max(df$order) - 12),                         rangeslider = list(type = "date", thickness = 0.04))) %>%
  onRender("
    function(el) {
...
df_paro_se <- df_paro_se %>% mutate(sector = recode(sector, "Sin actividad económica" = "Sin actividad<br>económica"),
                                    sector = recode(sector, "Resto de servicios" = "Resto de<br>servicios"))

df_paro_se_mun <- df_paro_se %>%
  filter(mun == params$id_municipio & periodo == mes_actual$periodo & sector != "TOTAL" & sexo != "AMBOS SEXOS") %>% select(!mun)
df_paro_Canarias <- df_paro_se %>%
  filter(mun == "ES70" & periodo == mes_actual$periodo & sector != "TOTAL" & sexo != "AMBOS SEXOS") %>% 
  select(!mun)
...
df_paro_sexo_est_mun <- df_paro_sexo_est %>%
  filter(mun == params$id_municipio & periodo == mes_actual$periodo & estudios != "TOTAL" & substring(estudios, 1, 1) != " " & sexo == "AMBOS SEXOS") %>%

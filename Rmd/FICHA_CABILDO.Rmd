://www.gobiernodecanarias.org/istac/jaxi-istac/tabla.do?accion=jsonMtd&uuidConsulta=7b167ec2-df8a-405a-bc12-d5e10f1ed6f7
output:
  html_document:
    df_print: paged
...
data_electores <- fromJSON(params$url.cabildo_electores)
...
convocatoria <- convocatoria %>% 
  transform(convocatoria_formatted = paste0(unlist(lapply(strsplit(convocatoria, " "), '[[', 2))),
            url = paste0("url.cabildo_", substring(code, 0,4)))
...
if(params$convocatoria == 2019){
  df_cabildo <- data.frame(
    ...
    medida = data_cabildo[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[3]][["code"]],
    valor = unlist(strsplit(data_cabildo[["data"]][["observations"]], split = " | ", fixed = TRUE)))
  ...
  for(i in 1:data_cabildo[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["total"]][1]) {
    ...
    if(params$convocatoria %in% c(2015, 2011)){
      df_cabildo <- data.frame(
...
df_cabildo <- df_cabildo %>% transform(mun = if_else(mun == "38013_2007",  "38013", mun))
```

```{r Resultados}
df_cabildo_mun <- df_cabildo %>% 
  left_join(df_partido, by = "candidatura") %>%
  filter(mun == params$id_municipio)

partido_mas_votado <- df_cabildo_mun %>% filter(medida == "VOTOS_VALIDOS_CANDIDATURA" & substring(candidatura, 0,1) == "P") %>% top_n(1, valor) %>%
  select(name_partido) %>% as.character()
df_plot_ <- df_cabildo_mun %>% filter(substring(candidatura, 0,1) == "G") %>% arrange(valor) %>% select(c(medida, name_partido, valor))
df_plot_ <- df_plot_ %>%
...
g_cabildo_mun <- plot_ly(df_plot, labels = df_plot$name_partido, values = df_plot$porcentaje, type = 'pie', sort = TRUE, direction = "clockwise", hole = 0.6,
...
df_cabildo_C <- df_cabildo %>% 
  left_join(df_partido, by = "candidatura") %>%
  filter(mun == "ES70")

partido_mas_votado_C <- df_cabildo_C %>%
  filter(medida == "VOTOS_VALIDOS_CANDIDATURA" & substring(candidatura, 0,1) == "P") %>% top_n(1, valor) %>% select(name_partido) %>% as.character()
df_plot_C_ <- df_cabildo_C %>% filter(substring(candidatura, 0,1) == "G") %>% arrange(valor) %>% select(c(medida, name_partido, valor))
...
g_cabildo_C <- plot_ly(df_plot_C, labels = df_plot_C$name_partido, values = df_plot_C$porcentaje, type = 'pie', sort = TRUE, direction = "clockwise", hole = 0.6,
                           textposition = "none", textinfo = "none", insidetextfont = list(color = "white"), hoverinfo = "text", text = df_plot_C$hovertext,
                           rotation = 90, marker = list(colors = col, line = list(width = 0))
                           )

g_cabildo_C <- ggplotly(g_cabildo_C, tooltip = c("text")) %>%
  layout(showlegend = FALSE) %>%
  config(displaylogo = FALSE, modeBarButtons = list(list("zoom2d"),list("pan2d"),list("resetScale2d"),list("toImage")))
```

...
```{r Convocatoria}
data_participacion <- fromJSON(params$url.cabildo_participacion)
...
convocatoria_actual <- convocatoria_cabildo %>% filter(substring(code, 9,12) == params$convocatoria)
...
min_code <- min(convocatoria_cabildo$code)
...
g_line <- ggplot(data = df, 
  ...
  scale_x_continuous(breaks = c(convocatoria_cabildo$order[convocatoria_actual$order:1]), labels = c(convocatoria_cabildo$convocatoria_formatted[convocatoria_actual$order:1])) +
...
df_plot_ <- df_cabildo_mun %>% filter(substring(candidatura, 0,1) == "P") %>% arrange(valor) %>% select(medida, candidatura, valor)
df_plot <- df_plot_ %>%
...
df_otros <- df_cabildo_mun %>% filter(substring(candidatura, 0,1) == "P" & candidatura %notin% df_plot$candidatura)
df_otros <- df_otros %>%
  spread(medida, valor) %>%
  select(candidatura, valor = VOTOS_VALIDOS_CANDIDATURA, porcentaje = RATIO_VOTOS_CANDIDATURA)
...
df_plot_I_ <- df_cabildo_I %>% filter(substring(candidatura, 0,1) == "P" & candidatura %in% df_plot$candidatura) %>% arrange(valor) %>% select(medida, candidatura, valor)
...
g_cabildo_mun <- ggplot(data = df_plot,

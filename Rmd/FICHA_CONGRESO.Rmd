...
convocatoria_congreso <- convocatoria %>% 
  ...
  transform(url = paste0("url.", tolower(substring(code, 0,15))))
...
df <- df %>% mutate(
    tooltip = ifelse(code == convocatoria_actual$code,
                     sprintf(leyenda.content, nombre, ifelse(is.na(valor), " -", paste0(format(valor, big.mark = ".", decimal.mark = ",", nsmall = 1), "%")),
                             ifelse(is.na(variacion), " -", paste0(format(variacion, big.mark = ".", decimal.mark = ",", nsmall = 1), " puntos porcentuales"))),
...
leyenda_poblacion <- df %>% filter(code == convocatoria_actual$code) %>%
  mutate(valor = ifelse(is.na(valor), " -", paste0(format(valor, big.mark = ".", decimal.mark = ",", nsmall = 1), "%")),
         variacion = ifelse(is.na(variacion), " -", paste0(format(variacion, big.mark = ".", decimal.mark = ",", nsmall = 1), " puntos porcentuales"))) %>%
...
if(params$convocatoria %in% c("2019_NOVIEMBRE", "2019_ABRIL", 2016, 2004, 2000)){
...
df_plot_ <- df_congreso_mun %>% filter(substring(candidatura, 0,1) == "P") %>% arrange(valor) %>% select(medida, candidatura, valor)
df_plot <- df_plot_ %>%
...
df_otros <- df_congreso_mun %>% filter(substring(candidatura, 0,1) == "P" & candidatura %notin% df_plot$candidatura)
df_otros <- df_otros %>%
  spread(medida, valor) %>%
  select(candidatura, valor = VOTOS_VALIDOS_CANDIDATURA, porcentaje = RATIO_VOTOS_CANDIDATURA)

df_plot <- rbind(df_plot, c("Otros", sum(df_otros$valor, na.rm = NA), sum(df_otros$porcentaje, na.rm = NA)))
df_plot$valor <- as.numeric(df_plot$valor)
df_plot$porcentaje <- as.numeric(df_plot$porcentaje)

df_plot$candidatura <- ordered(df_plot$candidatura, levels = df_plot$candidatura[nrow(df_plot):1])

df_congreso_P <- df_congreso %>% 
  left_join(df_partido, by = "candidatura") %>%
  filter(mun == municipio_actual$id_provincia)

df_plot_P_ <- df_congreso_P %>% filter(substring(candidatura, 0,1) == "P" & candidatura %in% df_plot$candidatura) %>% arrange(valor) %>% select(medida, candidatura, valor)
df_plot_P <- df_plot_P_ %>% 
  spread(medida, valor) %>% filter(!is.na(VOTOS_VALIDOS_CANDIDATURA)) %>% 
  select(candidatura, valor_P = VOTOS_VALIDOS_CANDIDATURA, porcentaje_P = RATIO_VOTOS_CANDIDATURA)
...
df_plot_P <- rbind(df_plot_P, c("Otros", sum(df_otros$valor_P, na.rm = NA), sum(df_otros$porcentaje_P, na.rm = NA)))
df_plot_P$valor_P <- as.numeric(df_plot_P$valor_P)
df_plot_P$porcentaje_P <- as.numeric(df_plot_P$porcentaje_P)

df_plot <- df_plot %>%
  left_join(df_partido, by = "candidatura") %>%
  left_join(df_plot_P, by = "candidatura") %>%
  mutate(label = "|") %>% 
  transform(name_partido = if_else(candidatura == "Otros",  "Resto de candidaturas", name_partido))
...
df_plot$name_partido <- ordered(df_plot$name_partido, levels = df_plot$name_partido[nrow(df_plot_P):1])

g_congreso_mun <- ggplot(data = df_plot,
                         aes(y = name_partido, label = label, name = NULL, text = hovertext)) +
  geom_bar(stat = "identity", fill = "#0072A2", alpha = 0.8, width = 0.6, aes(x = porcentaje)) +

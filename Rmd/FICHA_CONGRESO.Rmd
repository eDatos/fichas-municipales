...
Variacion_Porcentual <- df %>% filter(nombre == municipio_actual$municipio & code == convocatoria_actual$code) %>% select(Tasa) %>%
...
df_plot <- df_plot %>% transform(hovertext = paste0(
  ...
  "<br>", municipio_actual$provincia,": ", format(valor_P, big.mark = ".", decimal.mark = ","), " votos",
  " (", format(round(porcentaje_P, 1), big.mark = ".", decimal.mark = ","), "%)"
  ))
...
df_plot$candidatura <- ordered(df_plot$candidatura, levels = df_plot$candidatura[nrow(df_plot_P):1])

if(nrow(df_plot)/2 == 6){
      col <- c("#8CD2EA", "#2CBCE2", "#00A6DC", "#008BD0", "#0072A2", "#005980")
      }else if(nrow(df_plot_)/2 == 5){
        col <- c("#2CBCE2", "#00A6DC", "#008BD0", "#0072A2", "#005980")
        }else if(nrow(df_plot_)/2 == 4){
          col <- c("#00A6DC", "#008BD0", "#0072A2", "#005980")
          }

g_congreso_mun <- ggplot(data = df_plot,
                         aes(y = candidatura, fill = candidatura, label = label, name = NULL, text = hovertext)) +
...
if(Variacion_Porcentual == "NA"){
  cat(paste0(
    "<div class='column-2'>", get_indicator2('Tasa de</br>abstención', Tasa_abstencion, './img/percent-blue.png'), "</div><div class='column-2'>", get_indicator2('Electores', Num_Electores, './img/percent-blue.png'), "</div>"))
}else{
  cat(paste0(
    "<div class='column-3'>", get_indicator2('Tasa de</br>variación', paste0(Variacion_Porcentual, '%'), Imagen_Variacion), "</div><div class='column-3'>", get_indicator2('Tasa de</br>abstención', Tasa_abstencion, './img/percent-blue.png'), "</div><div class='column-3'>", get_indicator2('Electores', Num_Electores, './img/percent-blue.png'), "</div>"))
}
```
</div>
...
<div class="piechart_P">
  `r g_congreso_mun`
</div>

<div class="tablechart" style="margin-top: 20px; margin-left: -5px;">
<div class="legend-vertical-container">
```{r leyenda barras, results='asis'}
for (i in 1:(nrow(df_plot)/2)) {
  cat(paste0(
    "<div class='progressbar-legend'><div class='progress-bar-blue",i,"'></div><div class='sp-content-b'>", df_plot[i,]$name_partido, " (", format(round(df_plot[i,]$porcentaje, 1), big.mark = ".", decimal.mark = ",", nsmall = 1), "%)</div></div>"))
}
```
  <div class='progressbar-legend'>
  <div class='progress-bar-8' style="padding-left: 6px; margin-top: 0; color: #59656E; font-weight: 600;">|</div><div class='sp-content-b'>`r municipio_actual$provincia`</div></div>
</div>
</div>

...
df <- df %>% mutate(
  tooltip = ifelse(ano == params$ano,
                   ...
                   ifelse(mun == params$id_municipio,
                          sprintf(leyenda.content_I, nombre, paste0(valor_inicial_l, ' (',ano,')'), valor_l, tasa_l),
                          sprintf(leyenda.content, nombre, paste0(valor_l, ' (',ano,')'), tasa_l)
                          )
                   )
  )
...
g_line <- ggplot(data = df, aes(x = ano)) +
  ...
  scale_x_continuous(breaks = function(x){seq(from = min(df$ano), to = max(df$ano), by = as.integer((max(df$ano)-min(df$ano))/3))}) +
...
g_line <- ggplotly(g_line, tooltip = "text") %>%
  ...
  layout(hoverlabel = "", hovermode = 'x unified',
         xaxis = list(autorange = FALSE,
                      range = c(max(min(df$ano), params$ano - 18), params$ano),
...
df_lv <- df %>% filter(mun == params$id_municipio & ano == params$ano)
...
if(Variacion_Porcentual != "NA"){
  if(df_lv$tasa == 0){
    diferencia <- df_lv$valor[which(df_lv$ano == params$ano)] - df$valor[which(df$mun == params$id_municipio & df$ano == params$ano-1)]
...
df_ind_Ingresos <- data.frame(
  ...
  ano = names(ind_Ingresos[["dimension"]][["TIME"]][["representation"]][["index"]]),
...
Num_Ingresos_hab <- df_ind_Ingresos %>% filter(mun == params$id_municipio & ano == params$ano) %>% select(valor) %>%
...
df_ind_Gastos <- data.frame(
  ...
  ano = names(ind_Gastos[["dimension"]][["TIME"]][["representation"]][["index"]]),
...
Num_Gastos_hab <- df_ind_Gastos %>% filter(mun == params$id_municipio & ano == params$ano) %>% select(valor) %>%
  as.numeric %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character
```

```{r Diferencia entre el presupuesto ejecutado y el inicial}
GAP <- format(round(((df_lv$valor_M[df_lv$ano == params$ano]) - (df_lv$valor_inicial[df_lv$ano == params$ano])), 1), big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character
```
...
df_programa <- data.frame(
  id_programa = data_gastos_programas[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["value"]][[5]][["id"]],
  name_programa = vector(mode = "character", length = data_gastos_programas[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["total"]][5]))

for(i in 1:nrow(df_programa)) {
  df_programa$name_programa[i] <- data_gastos_programas[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["value"]][[5]][["name"]][["text"]][[i]][["value"]]
...
df_gastos_programas <- data.frame(
  mun = rep(data_gastos_programas[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[3]][["code"]],
            each = length(data_gastos_programas[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[5]][["code"]]) *
                   length(data_gastos_programas[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[6]][["code"]])),
  id_programa = rep(data_gastos_programas[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[5]][["code"]],
                      each = length(data_gastos_programas[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[6]][["code"]])),
  ano = data_gastos_programas[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[6]][["code"]],
...
df_gastos_programas_mun <- df_gastos_programas %>%
  filter(ano == params$ano & mun == params$id_municipio) %>%
  left_join(df_programa, by = "id_programa") %>%
  arrange(desc(valor)) %>%
  select(programa, name_programa, valor)
...
df_presupuestos <- data.frame(
  ano = rep(data_ind_presupuestos[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[1]][["code"]],
...
padding_left <- ifelse(nchar(GAP) > 3, 5, 10)
padding <- ifelse(nchar(GAP) > 3, 0, 5)

get_indicator2_GAP = function(label, value, image) {
  return(sprintf(" style='padding: 5px %spx;'><div class='indicator2'><div class='image'><img src='%s'></img></div><div class='indicator-content' style='padding-left: %spx;'><p class='value'>%s</p><p class='label'>%s</p></div></div>", padding, image, padding_left, value, label))
}
```
...
height_b <- ifelse(nchar(leyenda_poblacion$nombre[1]) > 27, 317, 290)
```

...
df <- related_mun %>%
  ...
  select(code = periodo, id, nombre, valor) %>%
...
g_line <- ggplotly(g_line, tooltip = "text") %>%
  ...
  layout(hoverlabel = "", hovermode = 'x unified',
         xaxis = list(autorange = FALSE,
                      range = c(max(0.8, nrow(df[df$nombre == municipio_actual$municipio, ]) - 5*4 - 0.2), 
...
ind_GM <- df %>% filter(id == params$id_municipio & code %in% c(periodo_actual$code, periodo_tva$code))

Num_GM_num <- ind_GM %>% filter(id == params$id_municipio & code == periodo_actual$code) %>% select(valor) %>% as.numeric()
Num_GM <- Num_GM_num %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
Imagen_Variacion <- ifelse(is.na(ind_GM %>% filter(id == params$id_municipio & code == periodo_tva$code) %>% select(valor) %>% as.numeric()),
                           "NA",
                           ifelse(signo(ind_GM %>% filter(id == params$id_municipio & code == periodo_actual$code) %>% select(tasa) %>% as.numeric()) == "más",
                                  './img/up.png', './img/down.png')
                           )
Variacion_Porcentual_GM <- ind_GM %>% filter(id == params$id_municipio & code == periodo_actual$code) %>% select(tasa) %>% as.numeric() %>% 
...
df_GM_ne <- df_GM_ne %>% mutate(estudios = recode(estudios, "Sin estudios o estudios primarios" = "Sin estudios<br>o primarios"),

...
convocatoria_autonomicas <- convocatoria %>% 
  ...
  transform(url = paste0("url.", tolower(substring(code, 0,18))))
convocatoria_autonomicas <- convocatoria_autonomicas %>% 
  mutate(order = as.integer(order(1:nrow(convocatoria_autonomicas), decreasing = TRUE)),
         convocatoria_formatted = ifelse(substring(code, 13,16) == "2019",
                                         paste0(substring(code, 18,18), tolower(substring(code, 19,27)), " ", substring(code, 13,16)),
                                         paste0(substring(code, 13,16))),
         convocatoria_axis = ifelse(substring(code, 13,16) == "2019",
                                    paste0(tolower(substring(code, 18,20)), " ", substring(code, 13,16)),
                                    paste0(substring(code, 13,16))))

convocatoria_actual <- convocatoria_autonomicas %>% filter(substring(code, 13,27) == params$convocatoria)
...
g_line <- ggplot(data = df, 
  ...
  scale_x_continuous(breaks = c(convocatoria_autonomicas$order[convocatoria_actual$order:1]), labels = c(convocatoria_autonomicas$convocatoria_axis[convocatoria_actual$order:1])) +
...
if(params$convocatoria %in% c(2015, 2007)){
  ...
  for(i in 1:data_autonomicas[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["total"]][3]) {
    ...
    if(params$convocatoria %in% c("2019_REGIONALES", "2019_INSULARES", 2011)){
      df_autonomicas <- data.frame(

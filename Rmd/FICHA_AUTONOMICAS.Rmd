...
g_line <- ggplot(data = df, 
  ...
  scale_x_continuous(breaks = c(convocatoria_autonomicas$order[convocatoria_actual$order:1]), labels = c(convocatoria_autonomicas$convocatoria_formatted[convocatoria_actual$order:1])) +
...
df_plot_C_ <- df_autonomicas_C %>% filter(substring(candidatura, 0,1) == "P" & candidatura %in% df_plot$candidatura) %>% arrange(valor) %>% select(medida, candidatura, valor)
df_plot_C <- df_plot_C_ %>% 
  spread(medida, valor) %>% filter(!is.na(VOTOS_VALIDOS_CANDIDATURA)) %>% 
  select(candidatura, valor_C = VOTOS_VALIDOS_CANDIDATURA, porcentaje_C = RATIO_VOTOS_CANDIDATURA)
df_plot_C$valor_C <- as.numeric(df_plot_C$valor_C)
df_plot_C$porcentaje_C <- as.numeric(df_plot_C$porcentaje_C)

df_otros <- df_autonomicas_C %>% filter(substring(candidatura, 0,1) == "P" & candidatura %notin% df_plot$candidatura)
df_otros <- df_otros %>%
  spread(medida, valor) %>%
  select(candidatura, valor_C = VOTOS_VALIDOS_CANDIDATURA, porcentaje_C = RATIO_VOTOS_CANDIDATURA)

df_plot_C <- rbind(df_plot_C, c("Otros", sum(df_otros$valor_C, na.rm = NA), sum(df_otros$porcentaje_C, na.rm = NA)))
df_plot_C$valor_C <- as.numeric(df_plot_C$valor_C)
df_plot_C$porcentaje_C <- as.numeric(df_plot_C$porcentaje_C)

df_plot <- df_plot %>%
  left_join(df_partido, by = "candidatura") %>%
  left_join(df_plot_C, by = "candidatura") %>%
  mutate(label = "|") %>% 
...
df_plot <- df_plot %>% transform(hovertext = paste0(
  ...
  "<br>Canarias: ", format(valor_C, big.mark = ".", decimal.mark = ","), " votos",
  " (", format(round(porcentaje_C, 1), big.mark = ".", decimal.mark = ","), "%)"
  ))
...
df_plot$candidatura <- ordered(df_plot$candidatura, levels = df_plot$candidatura[nrow(df_plot_C):1])
...
```
  ...
  <div class='progress-bar-8' style="padding-left: 6px; margin-top: 0; color: #59656E; font-weight: 600;">|</div><div class='sp-content-b'>Canarias</div></div>
</div>
</div>

...
```{r Convocatoria}
data_participacion <- fromJSON(params$url.autonomicas_participacion)
...
convocatoria_autonomicas <- convocatoria %>% 
  filter(substring(code, 0,12) == "AUTONOMICAS_") %>% 
  filter(code != "AUTONOMICAS_2019_REGIONALES") %>% 
...
convocatoria_actual <- convocatoria_autonomicas %>% filter(substring(code, 13,16) == params$convocatoria)
...
min_code <- min(convocatoria_autonomicas$code)
...
if(params$convocatoria %in% c(2015, 2007)){
  ...
  for(i in 1:data_autonomicas[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["total"]][3]) {
    ...
    if(params$convocatoria %in% c(2019, 2011)){
      df_autonomicas <- data.frame(
...
df_plot_ <- df_autonomicas_mun %>% filter(substring(candidatura, 0,1) == "P") %>% arrange(valor) %>% select(medida, candidatura, valor)
df_plot <- df_plot_ %>%
...
df_otros <- df_autonomicas_mun %>% filter(substring(candidatura, 0,1) == "P" & candidatura %notin% df_plot$candidatura)
df_otros <- df_otros %>%
  spread(medida, valor) %>%
  select(candidatura, valor = VOTOS_VALIDOS_CANDIDATURA, porcentaje = RATIO_VOTOS_CANDIDATURA)
...
df_plot_I_ <- df_autonomicas_I %>% filter(substring(candidatura, 0,1) == "P" & candidatura %in% df_plot$candidatura) %>% arrange(valor) %>% select(medida, candidatura, valor)

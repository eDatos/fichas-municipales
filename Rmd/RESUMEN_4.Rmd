...
g_line_el_s <- ggplot(data = df, aes(x = order, y = valor, group = nombre, colour = nombre, text = hovertext)) +
  geom_line(size = 0.7) +
  geom_text(aes(x = min_order+0.36, y = (0.92*I_Tasa_participacion_num), label = I_Tasa_participacion, colour = '#000000'), size = 3.5) +
  geom_text(aes(x = max_order-0.36, y = (0.94*Tasa_participacion_num), label = Tasa_participacion, colour = '#000000'), size = 3.5) +
  theme_minimal() +
...
df_c_ae_C <- data.frame(
  ...
  name_ae = vector(mode = "character", length = data_afil_c_ae_C[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["total"]][3]))

for(i in 1:nrow(df_c_ae_C)) {
  df_c_ae_C$name_ae[i] <- data_afil_c_ae_C[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["value"]][[3]][["name"]][["text"]][[i]][["value"]]
}
...
df_c_ae <- data.frame(
  ...
  name_ae = vector(mode = "character", length = data_afil_c_ae[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["total"]][3]))

for(i in 1:nrow(df_c_ae)) {
  df_c_ae$name_ae[i] <- data_afil_c_ae[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["value"]][[3]][["name"]][["text"]][[i]][["value"]]
}
...
g_c_afil_ae_sexo_mun <- ggplot(df_c_afil_ae_sexo_mun, aes(x = ae, fill = sexo, group = sexo, text = hovertext)) +
...
g_c_afil_ae_sexo_mun <- ggplotly(g_c_afil_ae_sexo_mun, tooltip = c("text")) %>%
...
tb_cot_ae <- as.data.frame(tb_cot_ae[nrow(tb_cot_ae):1,]) %>%
  rownames_to_column %>%
...
<div class="row" style="margin-top: 20px">
  ...
  <div class="column-2" style="height: 290px;">`r g_c_afil_ae_sexo_mun`</div>
</div>

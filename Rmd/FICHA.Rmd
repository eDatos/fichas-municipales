...
edad_media <- df_edadm %>% filter(mun == params$id_municipio & año == params$año) %>% subset(select = valor) %>% as.numeric %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
id_dep <- round(df_id_depen %>% filter(mun == params$id_municipio & año == params$año) %>% subset(select = valor), 1) %>% as.numeric %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
df_crec_veg_mun <- df_crec_veg %>% filter(año == params$año & mun == params$id_municipio)

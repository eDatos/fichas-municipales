...
leyenda.template <- "<div class='serie-placeholder serie-placeholder-%s'><div class='sp-bullet' style='background-color: %s'></div><div class='sp-content'><span class='sp-municipio'>%s</span><br/>Habitantes: <span class='sp-habitantes'>%s</span><br/>Tasa de crecimiento: <span class='sp-tasa'>%s</span></div></div>"
leyenda.content <- "<span class='sp-municipio'>%s</span><br/>Habitantes: <span class='sp-habitantes'>%s</span><br/>Tasa de crecimiento: <span class='sp-tasa'>%s</span>"

df <- df %>% mutate(
    tooltip = ifelse(Año == params$año,
                     sprintf(leyenda.content, Municipio, ifelse(is.na(Valor), " -", format(Valor, big.mark = ".", decimal.mark = ",")),
                             ifelse(is.na(Tasa), " -", format(paste0(round(Tasa,1), "%"), big.mark = ".", decimal.mark = ",", nsmall = 1))),
...
leyenda_poblacion <- df %>% filter(Año == params$año) %>%
  mutate(Valor = ifelse(is.na(Valor), " -", format(Valor, big.mark = ".", decimal.mark = ",")),
         Tasa = ifelse(is.na(Tasa), " -", format(paste0(round(Tasa,1), "%"), big.mark = ".", decimal.mark = ",", nsmall = 1))) %>%
...
percent_canarias <- format(round(df_Pobl[which(df_Pobl$id==params$id_municipio),]$poblacion / sum(df_Pobl$poblacion) * 100, 1), big.mark = ".", decimal.mark = ",", nsmall = 1)
...
percent_isla <- format(round(df_Pobl_isla[which(df_Pobl_isla$id==params$id_municipio),]$poblacion / sum(df_Pobl_isla$poblacion) * 100, 1), big.mark = ".", decimal.mark = ",", nsmall = 1)
...
if(Variacion_Porcentual_Pobl == "NA"){
  ...
  cat(paste0(
    "<div class='column-4'>", get_indicator2('Tasa de</br>', paste0(Variacion_Porcentual_Pobl, '%'), Imagen_Variacion), "</div><div class='column-4'>", get_indicator2('Edad</br>media', edad_media, './img/family.png'), "</div><div class='column-4'>", get_indicator2(paste0('Mujeres</p><p class="label" style="color: #999;">', Num_Mujeres), paste0(Percent_Mujeres, '%'), './img/female.png'), "</div><div class='column-4'>", get_indicator2(paste0('Hombres</p><p class="label" style="color: #999;">', Num_Hombres), paste0(Percent_Hombres, '%'), './img/male.png'), "</div>"))
}
```
</div>
...
<div class="row" style="margin-top: 10px">
  <div class="column-2">`r get_indicator3("Española", paste0(peso_españoles, "%"),"./img/spain2.png")`</div> 
  <div class="column-2">`r get_indicator3("Extranjera", paste0(peso_extranjeros, "%"),"./img/world.png")`</div>
</div>
</div>
...
<div class="row highlight">
  ...
  <div class ="column-2">`r get_indicator_met("Crecimiento vegetativo", "Diferencia entre nacimientos y defunciones.","./img/balanza.png")`</div>
  ...
  <div class ="column-2">`r get_indicator_met("Tasa de natalidad", "Total de nacimientos de madre perteneciente a un determinado ámbito en un año concreto por cada 1.000 habitantes.","./img/natalidad.png")`</div>
  <div class ="column-2">`r get_indicator_met("Tasa de mortalidad", "Número de defunciones a lo largo del año frente al total poblacional del ámbito geográfico correspondiente por cada 1.000 habitantes de ese ámbito.","./img/muerte.png")`</div>
  <div class ="column-2">`r get_indicator_met("Índice de dependencia", "Cociente entre la población perteneciente a un determinado ámbito a 1 de enero de un año concreto menor de 16 años o mayor de 64 entre la población de 16 a 64 años, expresado en tanto por cien.","./img/dependencia.png")`</div>
</div>

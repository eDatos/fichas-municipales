...
colnames(df_afil_) <- c('mun', 'sl', 'nacionalidad',  'periodo', 'sexo', 'valor')
...
df_afil_ <- df_afil_ %>% filter(substring(periodo, 5,8) %in% c("M03", "M06", "M09", "M12") & substring(mun, 0,2) != "ES")

df_u <- df_afil_ %>% filter(sl == "TOTAL DE AFILIACIONES" & sexo == "AMBOS SEXOS" & nacionalidad == "TOTAL") %>%
  select(c(mun, periodo, valor)) %>% arrange(periodo) %>% 
  transform(periodo = ifelse(grepl("M", periodo, fixed = TRUE), gsub("M", "-", periodo)))
...
df <- related_mun %>%
  ...
  filter(code <= mes_actual$code & order >= 60)
...
df <- df %>% mutate(periodo = strsplit(periodo, " ")) %>% 
  ...
  mutate(
    ...
    tooltip = ifelse(periodo == mes_actual$periodo_formatted, 
                     sprintf(leyenda.content, nombre, format(valor, big.mark = ".", decimal.mark = ","), format(tasa, big.mark = ".", decimal.mark = ",")),
                     sprintf(leyenda.content, nombre, paste0(format(valor, big.mark = ".", decimal.mark = ","), ' (',periodo,')'), format(tasa, big.mark = ".", decimal.mark = ","))
              ),
...
df_afil_ <- cbind(data.frame(do.call('rbind', data_afil[["data"]][["dimCodes"]])), data_afil[["data"]][["Valor"]])
colnames(df_afil_) <- c('mun', 'sl', 'nacionalidad',  'code', 'sexo', 'valor')
df_afil_$mun <- factor(df_afil_$mun, levels=data_afil[["categories"]][["codes"]][[1]], labels=data_afil[["categories"]][["labels"]][[1]])
df_afil_$sl <- factor(df_afil_$sl, levels=data_afil[["categories"]][["codes"]][[2]], labels=data_afil[["categories"]][["labels"]][[2]])
df_afil_$nacionalidad <- factor(df_afil_$nacionalidad, levels=data_afil[["categories"]][["codes"]][[3]], labels=data_afil[["categories"]][["labels"]][[3]])
df_afil_$code <- factor(df_afil_$code, levels=data_afil[["categories"]][["codes"]][[4]])
df_afil_$sexo <- factor(df_afil_$sexo, levels=data_afil[["categories"]][["codes"]][[5]], labels=data_afil[["categories"]][["labels"]][[5]])
df_afil_$valor <- as.numeric(df_afil_$valor)
df_afil_$mun <- recode_mun(df_afil_, 'mun')
df_afil_ <- df_afil_ %>% filter(substring(code, 5,8) %in% c("M03", "M06", "M09", "M12")) %>% 
  transform(code = ifelse(grepl("M", code, fixed = TRUE), gsub("M", "-", code)))
...
for (i in 1:nrow(leyenda_poblacion)) {
  cat(sprintf(template, i, leyenda_poblacion$color[i], leyenda_poblacion$nombre[i], leyenda_poblacion$valor[i], leyenda_poblacion$tasa[i]))
}
```

...
df_c_ae_C <- data.frame(
  ...
  name_ae = vector(mode = "character", length = data_afil_ae_C[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["total"]][3]))

for(i in 1:nrow(df_c_ae_C)) {
  df_c_ae_C$name_ae[i] <- data_afil_ae_C[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["value"]][[3]][["name"]][["text"]][[i]][["value"]]
}

df_ae <- read.csv2("../CL_CNAE.csv", encoding = "UTF-8", sep = ",")
df_ae <- df_ae %>% select(!name_ae)

df_c_ae_C <- df_c_ae_C %>% inner_join(df_ae, by = "id_ae")
...
df_c_ae <- data.frame(
  ...
  name_ae = vector(mode = "character", length = data_afil_ae[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["total"]][3]))

for(i in 1:nrow(df_c_ae)) {
  df_c_ae$name_ae[i] <- data_afil_ae[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["value"]][[3]][["name"]][["text"]][[i]][["value"]]
}

df_c_ae <- df_c_ae %>% inner_join(df_ae, by = "id_ae")

...
Imagen_Variacion <- ifelse(is.na(ind_viaj$valor[which(ind_viaj$...1 == params$año-1)]),
                           "NA",
                           ifelse(signo(ind_viaj %>% filter(id == params$id_municipio & code == mes_actual$code) %>% select(tasa) %>% as.numeric()) == "más",
                                  './img/up.png', './img/down.png')
                           )
Variacion_Porcentual_Viajeros <- ind_viaj %>% filter(id == params$id_municipio & code == mes_actual$code) %>% select(tasa) %>% as.numeric() %>%
...
df_hoteles_mun <- df_hoteles %>%
  ...
  select(c(ind, valor_hoteles = valor))
...
df_extrahoteleros_mun <- df_extrahoteleros %>%
  filter(cat == "TOTAL CATEGORÍAS" & mun == municipio_actual$municipio & periodo == mes_actual$periodo) %>% 
  select(c(ind, valor_extrahoteleros = valor))

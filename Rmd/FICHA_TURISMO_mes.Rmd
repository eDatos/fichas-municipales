...
params:
  ...
  # url.viajeros: https://datos.canarias.es/api/estadisticas/indicators/v1.0/indicators/ALOJATUR_VIAJEROS_ENTRADOS/data?representation=MEASURE%5BABSOLUTE%7CANNUAL_PERCENTAGE_RATE%7CANNUAL_PUNTUAL_RATE%5D&granularity=GEOGRAPHICAL%5BMUNICIPALITIES%5D%2CTIME%5BMONTHLY%5D
...
# df_viajeros_lugar$mun <- factor(df_viajeros_lugar$mun, levels=viajeros_lugar[["categories"]][["codes"]][[2]], labels=viajeros_lugar[["categories"]][["labels"]][[2]])
df_u_$Periodo <- paste0(substring(df_u_$Periodo, 0,4), "-", substring(df_u_$Periodo, 5,7))
df_u_$Valor <- as.numeric(df_u_$Valor)
# df_viajeros_lugar$mun <- recode_mun(df_viajeros_lugar, 'mun')

# # Viajeros entrados en alojamientos turísticos. 2009-M01
# data_u <- fromJSON(params$url.viajeros)
# df_u <- data.frame(
...
#   Periodo = rep(names(data_u[["dimension"]][["TIME"]][["representation"]][["index"]]),
#              each=length(data_u[["dimension"]][["MEASURE"]][["representation"]][["index"]])),
#   Ind = rep(names(data_u[["dimension"]][["MEASURE"]][["representation"]][["index"]]),
...
df <- related_mun %>%
  ...
  transform(Tasa = ifelse(substring(code, 0,4) != min(substring(code, 0,4)), round(100*((Valor / lag(Valor, n = 12)) - 1), 1), NA)
            # , Variacion = ifelse(substring(code, 0,4) != min(substring(code, 0,4)), Valor - lag(Valor, n = 12), NA)
            ) %>%
...
df <- df %>% mutate(Periodo = strsplit(Periodo, " ")) %>% 
  ...
  mutate(
    Periodo = paste0(...2, ' ', ...1),
    tooltip = ifelse(
      Periodo == mes_actual$periodo_formatted,
      sprintf(leyenda.content, Municipio, Valor_l, Tasa_l),
      sprintf(leyenda.content, Municipio, paste0(Valor_l, ' (',Periodo,')'), Tasa_l)
              )
  )

g_line <- ggplot(data = df, aes(x = reorder(code, order), y = Valor, group = Municipio, colour = Municipio, text = tooltip)) +
  geom_line(size = 0.7) +
  theme_minimal() +
  ...
  scale_x_discrete(breaks = function(x){paste0(2009:2021,"-M01")}, labels = function(x){substr(x, 0, 4)}) +
...
g_line <- ggplotly(g_line, tooltip = "text") %>%
  ...
  layout(hoverlabel = "", hovermode = 'x unified',
         xaxis = list(autorange = FALSE,
                      range = c(max(0.8, nrow(df[df$Municipio == municipio_actual$municipio, ]) - 5*12),
                                # df$order[df$code == mes_actual$code & df$Municipio == municipio_actual$municipio]
...
ind_viaj <- df %>% filter(Municipio == municipio_actual$municipio & code %in% c(mes_actual$code, mes_tva$code))

Num_Viajeros_num <- ind_viaj %>% filter(Municipio == municipio_actual$municipio & code == mes_actual$code) %>% select(Valor) %>% as.numeric()
Num_Viajeros <- Num_Viajeros_num %>% format(big.mark = ".", decimal.mark = ",")
# Variacion_Viajeros <- abs(ind_viaj %>% filter(Municipio == municipio_actual$municipio & code == mes_actual$code) %>% select(Variacion) %>% as.numeric()) %>%
#                       format(big.mark = ".", decimal.mark = ",")
# Signo_Variacion <- signo(ind_viaj %>% filter(Municipio == municipio_actual$municipio & code == mes_actual$code) %>% select(Variacion) %>% as.numeric())
Imagen_Variacion <- ifelse(is.na(ind_viaj$Valor[which(ind_viaj$...1 == params$año-1)]),
                           "NA",
                           ifelse(signo(ind_viaj %>% filter(Municipio == municipio_actual$municipio & code == mes_actual$code) %>% select(Tasa) %>% as.numeric()) == "más",
                                  './img/up.png', './img/down.png')
                           )
Variacion_Porcentual_Viajeros <- ind_viaj %>% filter(Municipio == municipio_actual$municipio & code == mes_actual$code) %>% select(Tasa) %>% as.numeric() %>%
...
df_estancia_media <- data.frame(
  id_municipio = rep(names(estancia_media[["dimension"]][["GEOGRAPHICAL"]][["representation"]][["index"]]),
...
Num_estancia_media <- df_estancia_media %>% filter(mes == mes_actual$code & id_municipio == params$id_municipio) %>% select(valor) %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character()
```
...
df_tasa_ocupacion <- data.frame(
  id_municipio = rep(names(tasa_ocupacion[["dimension"]][["GEOGRAPHICAL"]][["representation"]][["index"]]),
...
Tasa_ocupacion <- df_tasa_ocupacion %>% filter(mes == mes_actual$code & id_municipio == params$id_municipio) %>% select(valor) %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character()
```
...
# percent_canarias <- format(round(df_tur_mun$valor / sum(df_ranking$valor, na.rm = TRUE) * 100, 1), big.mark = ".", decimal.mark = ",")
...
# percent_isla <- format(round(df_tur_mun$valor / sum(df_tur_isla$valor, na.rm = TRUE) * 100, 1), big.mark = ".", decimal.mark = ",")
percent_isla <- format(round(df_tur_mun$Valor / num_Isla_total * 100, 1), big.mark = ".", decimal.mark = ",", nsmall = 1)
```

```{r Barras Lugar}
df_viajeros_lugar <- cbind(data.frame(do.call('rbind', viajeros_lugar[["data"]][["dimCodes"]])), viajeros_lugar[["data"]][["Valor"]])
colnames(df_viajeros_lugar) <- c('ind', 'lugar', 'mun', 'periodo', 'valor')
df_viajeros_lugar$ind <- factor(df_viajeros_lugar$ind, levels=viajeros_lugar[["categories"]][["codes"]][[1]], labels=viajeros_lugar[["categories"]][["labels"]][[1]])
df_viajeros_lugar$lugar <- factor(df_viajeros_lugar$lugar, levels=viajeros_lugar[["categories"]][["codes"]][[2]], labels=viajeros_lugar[["categories"]][["labels"]][[2]])
df_viajeros_lugar$mun <- factor(df_viajeros_lugar$mun, levels=viajeros_lugar[["categories"]][["codes"]][[3]], labels=viajeros_lugar[["categories"]][["labels"]][[3]])
df_viajeros_lugar$periodo <- paste0(substring(df_viajeros_lugar$periodo, 0,4), "-", substring(df_viajeros_lugar$periodo, 5,7))
df_viajeros_lugar$valor <- as.numeric(df_viajeros_lugar$valor)
df_viajeros_lugar$mun <- recode_mun(df_viajeros_lugar, 'mun')
df_viajeros_lugar <- df_viajeros_lugar %>% mutate(lugar = recode(lugar, " TOTAL RESIDENTES EN ESPAÑA" = "España"))

rk_viajeros_lugar_mun <- df_viajeros_lugar %>%
  filter(mun == municipio_actual$municipio & periodo == mes_actual$code & ind == "Viajeros alojados" &
...
df_viajeros_lugar_mun <- df_viajeros_lugar %>%
  filter(mun == municipio_actual$municipio & periodo == mes_actual$code & ind == "Viajeros alojados" &
    lugar %notin% c("TOTAL LUGARES DE RESIDENCIA", " TOTAL RESIDENTES EN EL EXTRANJERO", "  Canarias", "  Residentes en España excepto Canarias")) %>%
  select(c(lugar, valor))
df_viajeros_lugar_C <- df_viajeros_lugar %>%
  filter(mun == "CANARIAS" & periodo == mes_actual$code & ind == "Viajeros alojados" &
    lugar %notin% c("TOTAL LUGARES DE RESIDENCIA", " TOTAL RESIDENTES EN EL EXTRANJERO", "  Canarias", "  Residentes en España excepto Canarias")) %>%
  select(c(lugar, valor))
...
# df_viajeros_lugar_plot$lugar <- ordered(df_viajeros_lugar_plot$lugar, levels = df_viajeros_lugar_plot$lugar[12:1])
...
```{r Indicadores Lugar}
df_viajeros_nacionalidad_mun <- df_viajeros_lugar %>%
  filter(mun == municipio_actual$municipio & periodo == mes_actual$code & ind == "Viajeros alojados" & lugar %in% c("España", " TOTAL RESIDENTES EN EL EXTRANJERO")) %>%
  select(c(lugar, valor))
...
# Num_española <- df_viajeros_nacionalidad_mun$valor[which(df_viajeros_nacionalidad_mun$lugar == "España")] %>% format(big.mark = ".", decimal.mark = ",") %>% as.character()
# Num_extranjera <- df_viajeros_nacionalidad_mun$valor[which(df_viajeros_nacionalidad_mun$lugar == " TOTAL RESIDENTES EN EL EXTRANJERO")] %>% format(big.mark = ".", decimal.mark = ",") %>% as.character()
...
df_viajeros_canarios_mun <- df_viajeros_lugar %>%
  filter(mun == municipio_actual$municipio & periodo == mes_actual$code & ind == "Viajeros alojados" &
         lugar %in% c("  Canarias", "España", "TOTAL LUGARES DE RESIDENCIA")) %>%
  select(c(lugar, valor))
...
# Num_Canarias_Esp <- df_viajeros_canarios_mun$valor[which(df_viajeros_canarios_mun$lugar == "España")] %>% format(big.mark = ".", decimal.mark = ",") %>% as.character()
# Num_Canarias_Tot <- df_viajeros_canarios_mun$valor[which(df_viajeros_canarios_mun$lugar == "TOTAL LUGARES DE RESIDENCIA")] %>% format(big.mark = ".", decimal.mark = ",") %>% as.character()
...
df_aloj_isla$periodo <- paste0(substring(df_aloj_isla$periodo, 0,4), "-", substring(df_aloj_isla$periodo, 5,7))
...
num_turistas_canarios_mun <- df_viajeros_lugar %>%
  filter(mun == municipio_actual$municipio & periodo == mes_actual$code & ind == "Viajeros alojados" & lugar == "  Canarias") %>% select(valor) %>% as.numeric()
num_turistas_canarios_C <- df_viajeros_lugar %>%
  filter(mun == "CANARIAS" & periodo == mes_actual$code & ind == "Viajeros alojados" & lugar == "  Canarias") %>% select(valor) %>% as.numeric()
rk_aloj_isla_mun <- df_aloj_isla %>%
  ...
  arrange(valor) %>%
  select(c(lugar, valor))
rk_aloj_isla_mun <- rk_aloj_isla_mun %>% mutate(porcentaje = rk_aloj_isla_mun$valor/ num_turistas_canarios_mun * 100)
rk_aloj_isla_mun$lugar <- ordered(rk_aloj_isla_mun$lugar, levels = rk_aloj_isla_mun$lugar)

df_aloj_isla_mun <- df_aloj_isla %>%
  filter(mun == municipio_actual$municipio & periodo == mes_actual$code & ind == "Viajeros alojados" & lugar != "Canarias") %>% select(c(lugar, valor))
df_aloj_isla_C <- df_aloj_isla %>%
  filter(mun == "CANARIAS" & periodo == mes_actual$code & ind == "Viajeros alojados" & lugar != "Canarias") %>% select(c(lugar, valor))
df_aloj_isla_mun <- df_aloj_isla_mun %>%
  mutate(porcentaje = df_aloj_isla_mun$valor / num_turistas_canarios_mun * 100,
         valor_Canarias = df_aloj_isla_C$valor,
         porcentaje_Canarias = df_aloj_isla_C$valor / num_turistas_canarios_C * 100)

 df_aloj_isla_plot <- df_aloj_isla_mun %>%
  mutate(label = "|")
df_aloj_isla_plot$lugar <- ordered(df_aloj_isla_plot$lugar, levels = rk_aloj_isla_mun$lugar)
# df_aloj_isla_plot$lugar <- ordered(df_aloj_isla_plot$lugar, levels = df_aloj_isla_plot$lugar[7:1])
...
<h2 style="color: #666; margin: 0;">Indicadores turísticos mensuales</h2>
<hr>
...
<div class="row">
```{r, results='asis'}
if(Variacion_Porcentual_Viajeros == "NA"){
  cat(paste0(
    "<div class='column-3'>", get_indicator2('Estancia</br>media', Num_estancia_media, './img/cama.png'), "</div><div class='column-3'>", get_indicator2('Tasa de<br>ocupación', Tasa_ocupacion,'./img/percent-blue.png'), "</div><div class='column-3'>", get_indicator2('Tarifa media<br>diaria', ADR, './img/euro.png'), "</div>"))
}else{
  cat(paste0(
    "<div class='column-4'>", get_indicator2('Tasa de</br>variación', paste0(Variacion_Porcentual_Viajeros, '%'), Imagen_Variacion), "</div><div class='column-4'>", get_indicator2('Estancia</br>media', Num_estancia_media, './img/cama.png'), "</div><div class='column-4'>", get_indicator2('Tasa de<br>ocupación', Tasa_ocupacion,'./img/percent-blue.png'), "</div><div class='column-4'>", get_indicator2('Tarifa media<br>diaria', ADR, './img/euro.png'), "</div>"))
}
```
</div>
...
<div class="indicator-map" style="margin-top: -5px;">
  <div class="indicator-content" style="margin-top: -10px; padding: 8px;">
  <!-- <div class="column-2" style="padding-left: 30px; margin-right: -30px;"><p class="label"><span class="value">`r Num_Hoteles` </span></p></div> -->
  <!-- <div class="column-2" style="margin-left: -30px;"><p class="label">hoteles</p></div> -->
  <!-- <div class="row"><p class="label"><span class="value">`r Num_Hoteles` </span> Hoteles</p></div> -->
  <div class="row"><p class="label"><span class="value">`r Num_Hoteles` </span> Hoteleros</p></div>
  <div class="row"><div class="label2" style="margin-top: 12px;"><span class="value2">`r Plazas_Hoteles` </span>plazas en <span class="value2">`r Num_Habitaciones` </span>habitaciones</div>
  </div>
</div>
</div>
<div class="indicator-map" style="margin-top: 14px;">
  <div class="indicator-content" style="margin-top: -10px; padding: 8px;">
  <!-- <div class="column-2" style="padding-left: 30px; margin-right: -30px;"><p class="label"><span class="value">`r Num_extrahoteleros` </span></p></div> -->
  <!-- <div class="column-2" style="margin-left: -30px; margin-right: 20px;"><p class="label">extrahoteleros</p></div> -->
  <div class="row"><p class="label"><span class="value">`r Num_extrahoteleros` </span> Extrahoteleros</p></div>
  <div class="row"><p class="label2" style="margin-top: 12px;"><span class="value2">`r Plazas_extrahoteleros` </span>plazas en <span class="value2">`r Num_Apartamentos` </span>apartamentos</p></div>
  </div>
</div>
</div>
</div>
...
<div class="column indicator-title">
  <!-- <h3>Turistas canarios por residencia</h3> -->
  <h3>Canarios por residencia</h3>
</div>
</div>
...
<div class="row" style="margin: 20px 0;">
  ...
  <!-- <div class="column-2">`r get_indicator3('España', peso_española, './img/percent-black.png')`</div>  -->
  <!-- <div class="column-2">`r get_indicator3('Extranjero', peso_extranjera, './img/percent-black.png')`</div> -->
</div>
...
<div class="row" style="margin: 14px 0;">
  <div class="column-2">`r get_indicator3('Canarias respecto</br>a España', peso_Canarias_Esp, './img/percent-black.png')`</div>
  <div class="column-2">`r get_indicator3('Canarias respecto</br>al total', peso_Canarias_Tot, './img/percent-black.png')`</div>
</div>
</div>

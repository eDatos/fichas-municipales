...
df_u <- cbind(data.frame(do.call('rbind', viajeros_lugar[["data"]][["dimCodes"]])), viajeros_lugar[["data"]][["Valor"]])
colnames(df_u) <- c('Ind', 'lugar', 'mun', 'Periodo', 'Valor')
df_u$Ind <- factor(df_u$Ind, levels=viajeros_lugar[["categories"]][["codes"]][[1]], labels=viajeros_lugar[["categories"]][["labels"]][[1]])
...
df_u$Periodo <- paste0(substring(df_u$Periodo, 0,4), "-", substring(df_u$Periodo, 5,7))
...
leyenda.template <- "<div class='serie-placeholder serie-placeholder-%s'><div class='sp-bullet' style='background-color: %s'></div><div class='sp-content'><span class='sp-municipio'>%s</span><br/>Personas alojadas: <span class='sp-habitantes'>%s</span><br/>Tasa de crecimiento: <span class='sp-tasa'>%s&percnt;</span></div></div>"
leyenda.content <- "<span class='sp-municipio'>%s</span><br/>Personas alojadas: <span class='sp-habitantes'>%s</span><br/>Tasa de crecimiento: <span class='sp-tasa'>%s&percnt;</span>"
...
df <- related_mun %>%
  ...
  filter(substring(Periodo, 0,4) != "2009" & code <= mes_actual$code)
...
```{r Rankings}
df_ranking <- df_u %>%
  select(id = mun, periodo = Periodo, valor = Valor) %>%
...
df_viajeros_lugar_mun <- df_viajeros_lugar %>%
  filter(mun == municipio_actual$municipio & periodo == mes_actual$code & ind == "Viajeros alojados" &
    lugar %notin% c("TOTAL LUGARES DE RESIDENCIA", " TOTAL RESIDENTES EN EL EXTRANJERO", " TOTAL RESIDENTES EN ESPAÑA", "  Canarias", "  Residentes en España excepto Canarias")) %>%
...
df_viajeros_lugar_mun <- df_viajeros_lugar_mun %>%
  mutate(porcentaje = df_viajeros_lugar_mun$valor / sum(df_viajeros_lugar_mun$valor) * 100,
         valor_Canarias = df_viajeros_lugar_C$valor,
         porcentaje_Canarias = df_viajeros_lugar_C$valor / sum(df_viajeros_lugar_C$valor) * 100)
...
```{r Progress-bar Lugar}
df_viajeros_lugar_N_mun <- df_viajeros_lugar %>%
  filter(mun == municipio_actual$municipio & periodo == mes_actual$code & ind == "Viajeros alojados" &
         lugar %in% c("  Canarias", "  Residentes en España excepto Canarias", " TOTAL RESIDENTES EN EL EXTRANJERO")) %>%
  select(c(lugar, valor))
df_viajeros_lugar_N_mun <- df_viajeros_lugar_N_mun %>%
  mutate(porcentaje = df_viajeros_lugar_N_mun$valor / sum(df_viajeros_lugar_N_mun$valor) * 100)

porc_lugar_Canarias <- df_viajeros_lugar_N_mun$porcentaje[which(df_viajeros_lugar_N_mun$lugar == "  Canarias")]
porc_lugar_RestoEspaña <- df_viajeros_lugar_N_mun$porcentaje[which(df_viajeros_lugar_N_mun$lugar == "  Residentes en España excepto Canarias")]
porc_lugar_OtroPais <- df_viajeros_lugar_N_mun$porcentaje[which(df_viajeros_lugar_N_mun$lugar == " TOTAL RESIDENTES EN EL EXTRANJERO")]
...
df_aloj_isla_mun <- df_aloj_isla_mun %>%
  mutate(porcentaje = df_aloj_isla_mun$valor / sum(df_aloj_isla_mun$valor) * 100,
         valor_Canarias = df_aloj_isla_C$valor,
         porcentaje_Canarias = df_aloj_isla_C$valor / sum(df_aloj_isla_C$valor) * 100)
...
<div class="row">
<div class="column-4">`r get_indicator2(paste0(Variacion_Viajeros, " ", Signo_Variacion, " que<br>en ", mes_tva$periodo_formatted), paste0(Variacion_Porcentual_Viajeros, "%"), Imagen_Variacion)`</div>
<div class="column-4">`r get_indicator2("Estancia<br>media", Num_estancia_media,"./img/family.png")`</div>
<div class="column-4">`r get_indicator2("Tasa de<br>ocupación", paste0(Tasa_ocupacion, "%"),"./img/family.png")`</div>
<div class="column-4">`r get_indicator2("ADR", paste0(ADR, "€"),"./img/family.png")`</div>
</div>
...
<div class="piramide" style="height: 260px">
  `r g_viajeros_lugar`
</div>
...
<div class="column indicator-title" style="margin-top: 0px;">
  <h3>Extrahoteleros</h3>
</div>
</div>

<div class="row">
<div class="column highlight">
  <div class="column-3">`r get_indicator3("Número", Num_Hoteles,"./img/sinestudio.png")`</div>
  <div class="column-3">`r get_indicator3("Plazas", Plazas_Hoteles,"./img/secundarios.png")`</div>
  <div class="column-3">`r get_indicator3("Habitaciones", Num_Habitaciones,"./img/sinestudio.png")`</div>
</div>

<div class="column highlight">
  <div class="column-3">`r get_indicator3("Número", Num_extrahoteleros,"./img/primarios.png")`</div>
  <div class="column-3">`r get_indicator3("Plazas", Plazas_extrahoteleros,"./img/postsecundario.png")`</div>
  <div class="column-3">`r get_indicator3("Apartamentos", Num_Apartamentos,"./img/primarios.png")`</div>
</div>

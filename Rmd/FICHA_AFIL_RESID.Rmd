...
g_piramide <- ggplot(data = df_mun, aes(x = reorder(gredad, order), fill = sexo, group = sexo)) +
  ...
  scale_y_continuous(n.breaks = 8, labels = function(x){paste0(format(abs(x)*100, big.mark = '.', decimal.mark = ","), '%')},
...
# Num_afil_ext <- df_afil_extr_mun %>% filter(nacionalidad == " EXTRANJERA") %>% select(valor) %>% as.numeric()
...
# Num_afil_apat <- df_afil_extr_mun %>% filter(nacionalidad == " APÁTRIDA O SIN ESPECIFICAR") %>% select(peso) %>% as.numeric()
...
# Num_afil_asal <- df_afil_sl_mun %>% filter(sl == " Empleos asalariados") %>% select(valor) %>% as.numeric()
Peso_afil_auton <- df_afil_sl_mun %>% filter(sl == " Empleos autónomos") %>% select(peso) %>% as.numeric()
# Num_afil_auton <- df_afil_sl_mun %>% filter(sl == " Empleos autónomos") %>% select(valor) %>% as.numeric()
```

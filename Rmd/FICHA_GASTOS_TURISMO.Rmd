...
g_line <- ggplotly(g_line, tooltip = "text") %>%
  ...
  layout(hoverlabel = "", hovermode = 'x unified',
         xaxis = list(autorange = FALSE,
                      range = c(max(0.8, nrow(df[df$mun == params$id_municipio, ]) - 5*4 - 0.2), 
                                df$order[df$code == periodo_actual$code & df$mun == params$id_municipio]),
...
ind_GM <- df %>% filter(mun == params$id_municipio & code %in% c(periodo_actual$code, periodo_tva$code))

Num_GM_num <- ind_GM %>% filter(mun == params$id_municipio & code == periodo_actual$code) %>% select(valor) %>% as.numeric()
Num_GM <- Num_GM_num %>% format(big.mark = ".", decimal.mark = ",")
Imagen_Variacion <- ifelse(is.na(ind_GM %>% filter(mun == params$id_municipio & code == periodo_tva$code) %>% select(valor) %>% as.numeric()),
                           "NA",
                           ifelse(signo(ind_GM %>% filter(mun == params$id_municipio & code == periodo_actual$code) %>% select(tasa) %>% as.numeric()) == "más",
                                  './img/up.png', './img/down.png')
                           )
Variacion_Porcentual_GM <- ind_GM %>% filter(mun == params$id_municipio & code == periodo_actual$code) %>% select(tasa) %>% as.numeric() %>%

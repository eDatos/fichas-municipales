...
Num_GM_num <- ind_GM %>% filter(code == periodo_actual$code) %>% select(valor) %>% as.numeric()
Num_GM <- Num_GM_num %>% format(big.mark = ".", decimal.mark = ",")
Variacion_Porcentual <- ind_GM %>% filter(code == periodo_actual$code) %>% select(tasa) %>% as.numeric() %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)

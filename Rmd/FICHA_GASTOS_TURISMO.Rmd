...
# df_GM_sexo_edad$Municipio <- factor(df_GM_sexo_edad$Municipio, levels=GM_sexo_edad[["categories"]][["codes"]][[4]], labels=GM_sexo_edad[["categories"]][["labels"]][[4]])
# df_GM_sexo_edad$Periodo <- factor(df_GM_sexo_edad$Periodo, levels=GM_sexo_edad[["categories"]][["codes"]][[5]]), labels=GM_sexo_edad[["categories"]][["labels"]][[5]])
df_GM_sexo_edad$Valor <- as.numeric(df_GM_sexo_edad$Valor) %>% round(0)
# df_GM_sexo_edad$Municipio <- recode_mun(df_GM_sexo_edad, 'Municipio')
...
leyenda.template <- "<div class='serie-placeholder serie-placeholder-%s'><div class='sp-bullet' style='background-color: %s'></div><div class='sp-content'><span class='sp-municipio'>%s</span><br/>Gasto medio: <span class='sp-habitantes'>%s</span><br/>Tasa de crecimiento: <span class='sp-tasa'>%s</span></div></div>"
leyenda.content <- "<span class='sp-municipio'>%s</span><br/>Gasto medio: <span class='sp-habitantes'>%s</span><br/>Tasa de crecimiento: <span class='sp-tasa'>%s</span>"
...
g_line <- ggplot(data = df, aes(x = code, y = Valor, group = Municipio, colour = Municipio, text = tooltip)) +
  ...
  scale_x_discrete(breaks = function(x){paste0(2018:2021,"Q1")}, labels = function(x){substr(x, 0, 4)}) +
  scale_y_continuous(labels = function(x){paste0(format(x, big.mark = ".", decimal.mark = ","), "")}, n.breaks = 5)
  ...
  layout(hoverlabel = "", hovermode = 'x unified',
         xaxis = list(autorange = FALSE,
                      range = c(max(0.8, nrow(df[df$Municipio == municipio_actual$municipio, ]) - 5*4 - 0.2), 
                                ...
                                # nrow(df[df$Municipio == municipio_actual$municipio, ])
                                ),
...
Num_GM <- Num_GM_num %>% round(0) %>% format(big.mark = ".", decimal.mark = ",")
# Variacion_GM <- abs((ind_GM %>% filter(Municipio == municipio_actual$municipio & code == periodo_actual$code) %>% select(Valor) %>% as.numeric()) -
                      # (ind_GM %>% filter(Municipio == municipio_actual$municipio & code == periodo_tva$code) %>% select(Valor) %>% as.numeric())) %>%
                      # round(0) %>% format(big.mark = ".", decimal.mark = ",")
# Signo_Variacion <- signo((ind_GM %>% filter(Municipio == municipio_actual$municipio & code == periodo_actual$code) %>% select(Valor) %>% as.numeric()) -
                         # (ind_GM %>% filter(Municipio == municipio_actual$municipio & code == periodo_tva$code) %>% select(Valor) %>% as.numeric()))
...
Gasto_por_turista <- df_GM_mun$valor[which(df_GM_mun$ind == "Gasto por turista")] %>% round(0) %>% format(big.mark = ".", decimal.mark = ",")

df_GM_sexo_mun <- ind_GM_sexo_edad %>%
  filter(ind == "Gasto por turista" & sexo != "AMBOS SEXOS" & gredad == "TOTAL GRUPOS DE EDADES") %>% 
  select(c(sexo, valor))

Gasto_por_turista_M <- df_GM_sexo_mun$valor[which(df_GM_sexo_mun$sexo == "Mujeres")] %>% round(0) %>% format(big.mark = ".", decimal.mark = ",")
Gasto_por_turista_H <- df_GM_sexo_mun$valor[which(df_GM_sexo_mun$sexo == "Hombres")] %>% round(0) %>% format(big.mark = ".", decimal.mark = ",")

df_GM_edad_mun <- ind_GM_sexo_edad %>%
  filter(ind == "Gasto por turista" & sexo == "AMBOS SEXOS" & gredad != "TOTAL GRUPOS DE EDADES") %>% 
  select(c(gredad, valor))

Gasto_por_turista_menor <- df_GM_edad_mun$valor[which(df_GM_edad_mun$gredad == "De 16 a 44 años")] %>% round(0) %>% format(big.mark = ".", decimal.mark = ",")
Gasto_por_turista_mayor <- df_GM_edad_mun$valor[which(df_GM_edad_mun$gredad == "Mayor de 44 años")] %>% round(0) %>% format(big.mark = ".", decimal.mark = ",")
...
Gasto_por_turista_menor_C <- df_GM_sexo_edad_C$valor[which(df_GM_sexo_edad_C$gredad == "De 16 a 44 años")] %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
Gasto_por_turista_mayor_C <- df_GM_sexo_edad_C$valor[which(df_GM_sexo_edad_C$gredad == "Mayor de 44 años")] %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
# df_GM_sl$periodo <- factor(df_GM_sl$periodo, levels=GM_sl[["categories"]][["codes"]][[4]], labels=GM_sl[["categories"]][["labels"]][[4]])
df_GM_sl$valor <- as.numeric(df_GM_sl$valor)
...
Gasto_tur_sl_Act <- df_GM_sl_mun$valor[which(df_GM_sl_mun$sl == "Activos")] %>% round(0) %>% format(big.mark = ".", decimal.mark = ",")
Gasto_tur_sl_NAct <- df_GM_sl_mun$valor[which(df_GM_sl_mun$sl == "Inactivos")] %>% round(0) %>% format(big.mark = ".", decimal.mark = ",")
...
Gasto_tur_sl_Act_C <- df_GM_sl_C$valor[which(df_GM_sl_C$sl == "Activos")] %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
Gasto_tur_sl_NAct_C <- df_GM_sl_C$valor[which(df_GM_sl_C$sl == "Inactivos")] %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
# df_GM_ni$periodo <- factor(df_GM_ni$periodo, levels=GM_ni[["categories"]][["codes"]][[4]], labels=GM_ni[["categories"]][["labels"]][[4]])
...
g_GM_ni <- ggplot(df_GM_ni_plot, aes(y = nivel, fill = nivel, label = label, name = NULL, text = hovertext)) +
  geom_bar(stat = "identity", alpha = 0.8, width = 0.4, aes(x = valor)) +
...
# df_GM_ne$periodo <- factor(df_GM_ne$periodo, levels=GM_ne[["categories"]][["codes"]][[4]], labels=GM_ne[["categories"]][["labels"]][[4]])
df_GM_ne$valor <- as.numeric(df_GM_ne$valor)
...
df_GM_ne_mun$estudios <- ordered(df_GM_ne_mun$estudios, levels = df_GM_ne_mun$estudios[4:1])

df_GM_ne_mun <- df_GM_ne_mun %>% transform(hovertext = paste0(
    "<b>", estudios, "</b>",
    "<br>",trimws(format(valor, big.mark = ".", decimal.mark = ",")), " €", 
    "<br>Canarias: ", format(valor_Canarias, big.mark = ".", decimal.mark = ","), " €"
    )
...
df_GM_ne_plot <- df_GM_ne_mun %>%
  mutate(label = "|")
df_GM_ne_plot$estudios <- ordered(df_GM_ne_plot$estudios, levels = df_GM_ne_plot$estudios[4:1])
...
g_GM_ne <- ggplot(df_GM_ne_plot, aes(y = estudios, fill = estudios, label = label, name = NULL, text = hovertext)) +
  geom_bar(stat = "identity", alpha = 0.8, width = 0.4, aes(x = valor)) +
  geom_text(size = 6, color = "#59656E", aes(x = valor_Canarias, text = "")) +
  scale_fill_manual(values = c("Sin estudios o primarios" = "#005980", "Secundarios" = "#008BD0", "Superiores" = "#8CD2EA")) +
  theme_minimal() +
  guides(fill = guide_legend(title = " ")) +
  labs(title = "", x = NULL, y = NULL, colour = " ") +
...
# df_GM_ta$periodo <- factor(df_GM_ta$periodo, levels=GM_ta[["categories"]][["codes"]][[4]], labels=GM_ta[["categories"]][["labels"]][[4]])
df_GM_ta$valor <- as.numeric(df_GM_ta$valor)
...
Gasto_tur_ta_hot <- df_GM_ta_mun$valor[which(df_GM_ta_mun$tipo == "Hoteleros")] %>% round(0) %>% format(big.mark = ".", decimal.mark = ",")
Gasto_tur_ta_extr <- df_GM_ta_mun$valor[which(df_GM_ta_mun$tipo == "Extrahoteleros")] %>% round(0) %>% format(big.mark = ".", decimal.mark = ",")
Gasto_tur_ta_otr <- df_GM_ta_mun$valor[which(df_GM_ta_mun$tipo == "Otros alojamientos colectivos")] %>% round(0) %>% format(big.mark = ".", decimal.mark = ",")
```
...
# df_GM_np$periodo <- factor(df_GM_np$periodo, levels=GM_np[["categories"]][["codes"]][[4]], labels=GM_np[["categories"]][["labels"]][[4]])
df_GM_np$valor <- as.numeric(df_GM_np$valor)
...
df_GM_np_mun$noches <- ordered(df_GM_np_mun$noches, levels = df_GM_np_mun$noches[4:1])

df_GM_np_mun <- df_GM_np_mun %>% transform(hovertext = paste0(
    "<b>", noches, "</b>",
    "<br>",trimws(format(valor, big.mark = ".", decimal.mark = ",")), " €", 
    "<br>Canarias: ", format(valor_Canarias, big.mark = ".", decimal.mark = ","), " €"
    )
...
df_GM_np_plot <- df_GM_np_mun %>%
  mutate(label = "|")
df_GM_np_plot$noches <- ordered(df_GM_np_plot$noches, levels = df_GM_np_plot$noches[4:1])
...
g_GM_np <- ggplot(df_GM_np_plot, aes(y = noches, fill = noches, label = label, name = NULL, text = hovertext)) +
  geom_bar(stat = "identity", alpha = 0.8, width = 0.4, aes(x = valor)) +
  geom_text(size = 6, color = "#59656E", aes(x = valor_Canarias, text = "")) +
  scale_fill_manual(values = c("1 - 7" = "#005980", "8 - 15" = "#008BD0", "&gt; 15" = "#8CD2EA")) +
  theme_minimal() +
  guides(fill = guide_legend(title = " ")) +
  labs(title = "", x = NULL, y = NULL, colour = " ") +
  theme(axis.text.x = element_text(size = 8),
...
# df_GM_pt$periodo <- factor(df_GM_pt$periodo, levels=GM_pt[["categories"]][["codes"]][[3]], labels=GM_pt[["categories"]][["labels"]][[3]])
df_GM_pt$valor <- as.numeric(df_GM_pt$valor) %>% round(0)
df_GM_pt$mun <- recode_mun(df_GM_pt, 'mun')
df_GM_pt <- df_GM_pt %>% transform(paquete = paste0(substring(paquete, 0,1), tolower(substring(paquete, 2))))
...
df_GM_pt_mun <- df_GM_pt_mun %>% transform(hovertext = paste0(
    "<b>", paquete, "</b>",
    "<br>",trimws(format(valor, big.mark = ".", decimal.mark = ",")), " €", 
    "<br>Canarias: ", format(valor_Canarias, big.mark = ".", decimal.mark = ","), " €"
    )
...
g_GM_pt <- ggplot(df_GM_pt_plot, aes(y = paquete, fill = paquete, label = label, name = NULL, text = hovertext)) +
  geom_bar(stat = "identity", alpha = 0.8, width = 0.4, aes(x = valor)) +
  geom_text(size = 6, color = "#59656E", aes(x = valor_Canarias, text = "")) +
  scale_fill_manual(values = c("Total" = "#005980", "Con servicios extras" = "#008BD0", "Sin servicios extras" = "#8CD2EA")) +
  theme_minimal() +
  guides(fill = guide_legend(title = " ")) +
  labs(title = "", x = NULL, y = NULL, colour = " ") +
  theme(axis.text.x = element_text(size = 8),
...
# df_GM_pais$periodo <- factor(df_GM_pais$periodo, levels=GM_pais[["categories"]][["codes"]][[4]]), labels=GM_pais[["categories"]][["labels"]][[4]])
...
df_GM_pais_mun$pais <- ordered(df_GM_pais_mun$pais, levels = df_GM_pais_mun$pais[4:1])

df_GM_pais_mun <- df_GM_pais_mun %>% transform(hovertext = paste0(
    "<b>", pais, "</b>",
    "<br>",trimws(format(valor, big.mark = ".", decimal.mark = ",")), " €", 
    "<br>Canarias: ", format(valor_Canarias, big.mark = ".", decimal.mark = ","), " €"
    )
...
df_GM_pais_plot <- df_GM_pais_mun %>%
  mutate(label = "|")
df_GM_pais_plot$pais <- ordered(df_GM_pais_plot$pais, levels = df_GM_pais_plot$pais[4:1])
...
g_GM_pais <- ggplot(df_GM_pais_plot, aes(y = pais, fill = pais, label = label, name = NULL, text = hovertext)) +
  geom_bar(stat = "identity", alpha = 0.8, width = 0.4, aes(x = valor)) +
  geom_text(size = 6, color = "#59656E", aes(x = valor_Canarias, text = "")) +
  scale_fill_manual(values = c("Alemania" = "#005980", "España" = "#008BD0", "Reino Unido" = "#2CBCE2", "Otros" = "#8CD2EA")) +
  theme_minimal() +
  guides(fill = guide_legend(title = " ")) +
  labs(title = "", x = NULL, y = NULL, colour = " ") +
  theme(axis.text.x = element_text(),
...
if(Variacion_Porcentual_GM == "NA"){
  ...
  cat(paste0(
    '<div class="column-3">', get_indicator2("Tasa de</br>...", paste0(Variacion_Porcentual_GM, "%"), Imagen_Variacion), '</div><div class="column-3">', get_indicator2("Mujeres", paste0(Gasto_por_turista_M, " €"), "./img/female.png"), '</div><div class="column-3">', get_indicator2("Hombres", paste0(Gasto_por_turista_H, " €"), "./img/male.png"), '</div>'))
}
```
</div>
...
<div class="piramide" style="height: 220px;">
`r g_GM_pais`
</div>

<div class="progressbar-legend-container">
<div class='progressbar-legend'>
<div class='progress-bar-8' style="padding-left: 6px; margin-top: 0; color: #59656E; font-weight: 600;">|</div><div class='sp-content'>Canarias</div></div>
</div>
...
<div class="indicator-map">
  ...
  <div class='progressbar-legend'><div class='progress-bar-1' style='font-size: 11px;'></div><div class='sp-content'>`r df_GM_ne_plot$estudios[1]`</div></div>
  <div class='progressbar-legend'><div class='progress-bar-2' style='font-size: 11px;'></div><div class='sp-content'>`r df_GM_ne_plot$estudios[2]`</div></div>
  <div class='progressbar-legend'><div class='progress-bar-3' style='font-size: 11px;'></div><div class='sp-content'>`r df_GM_ne_plot$estudios[3]`</div></div>
  <div class='progressbar-legend'>
  <div class='progress-bar-8' style="padding-left: 6px; margin-top: 0; color: #59656E; font-weight: 600; font-size: 11px;">|</div><div class='sp-content'>Canarias</div></div>
  </div>
</div>
</div>
</div>
...
<div class="indicator-map">
  ...
  <div class="legend-vertical-container">
  <div class='progressbar-legend'><div class='progress-bar-1' style='font-size: 11px;'></div><div class='sp-content'>`r df_GM_pt_plot$paquete[1]`</div></div>
  <div class='progressbar-legend'><div class='progress-bar-2' style='font-size: 11px;'></div><div class='sp-content'>`r df_GM_pt_plot$paquete[2]`</div></div>
  <div class='progressbar-legend'><div class='progress-bar-3' style='font-size: 11px;'></div><div class='sp-content'>`r df_GM_pt_plot$paquete[3]`</div></div>
  <div class='progressbar-legend'>
  <div class='progress-bar-8' style="padding-left: 6px; margin-top: 0; color: #59656E; font-weight: 600; font-size: 11px;">|</div><div class='sp-content'>Canarias</div></div>
  </div>
</div>
...
<div class="indicator-map">
  ...
  <div class="legend-vertical-container">
  <div class='progressbar-legend'><div class='progress-bar-1' style='font-size: 11px;'></div><div class='sp-content'>`r df_GM_np_plot$noches[1]`</div></div>
  <div class='progressbar-legend'><div class='progress-bar-2' style='font-size: 11px;'></div><div class='sp-content'>`r df_GM_np_plot$noches[2]`</div></div>
  <div class='progressbar-legend'><div class='progress-bar-3' style='font-size: 11px;'></div><div class='sp-content'>`r df_GM_np_plot$noches[3]`</div></div>
  <div class='progressbar-legend'>
  <div class='progress-bar-8' style="padding-left: 6px; margin-top: 0; color: #59656E; font-weight: 600;font-size: 11px;">|</div><div class='sp-content'>Canarias</div></div>
  </div>
</div>
...
<div class="row highlight">
  <div style="padding: 10px;">`r get_indicator_met_("Gasto turístico", "Suma pagada por la adquisición de bienes y servicios de consumo y de objetos valiosos, para uso propio o para regalar, antes y durante los viajes turísticos; incluyendo los gastos incurridos por los propios visitantes, así como los gastos pagados o reembolsados por otros.")`</div>
  <div style="padding: 0 10px;">`r get_indicator_met("Tasa de ", "Incremento o disminución en el número de individuos pertenecientes a un determinado ámbito con respecto al mismo trimestre del año anterior al seleccionado por cada 1.000 habitantes.","./img/crecimiento.png")`</div>
  <div style="padding: 0 10px;">`r get_indicator_met("Paquete turístico", "Reserva previa del viaje que incluye al menos alojamiento y transporte, aunque alguno de estos dos elementos puede ser reemplazado por cualquier otro servicio que suponga una cantidad significativa del precio total del viaje (visitas organizadas, alquiler de coche, etc.) y que ha sido vendida a un precio global indivisible a través de agencia de viajes o turoperador.","./img/family2.png")`</div>
</div>

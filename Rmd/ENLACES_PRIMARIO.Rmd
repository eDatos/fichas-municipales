...
df_lenosos$cultivo <- factor(df_lenosos$cultivo, levels=data_lenosos[["categories"]][["codes"]][[2]], labels=data_lenosos[["categories"]][["labels"]][[2]])
...
sup_lenosos_num <- df_lenosos %>%
  filter(mun == params$id_municipio & ano == params$ano & cultivo == "TOTAL CULTIVOS LEÑOSOS" & sistema == "TOTAL EN PLANTACIÓN REGULAR (SECANO+REGADÍO)") %>% select(valor) %>% as.numeric
sup_lenosos <- sup_lenosos_num %>% format(big.mark = ".", decimal.mark = ",")
sup_lenosos_C <- df_lenosos %>%
  filter(mun == "ES70" & ano == params$ano & cultivo == "TOTAL CULTIVOS LEÑOSOS" & sistema == "TOTAL EN PLANTACIÓN REGULAR (SECANO+REGADÍO)") %>% select(valor) %>% as.numeric
...
df_lenosos <- df_lenosos %>%
  filter(substring(cultivo, 0,1) == " " & substring(cultivo, 0,4) != " Otr" & sistema == "TOTAL EN PLANTACIÓN REGULAR (SECANO+REGADÍO)" & ano == params$ano) %>% 
  transform(cultivo = trimws(cultivo))

rk_lenosos_mun <- df_lenosos %>%
  filter(mun == params$id_municipio) %>%
  top_n(8, valor) %>% 
  arrange(cultivo) %>%
  arrange(desc(valor)) %>%
...
df_lenosos_mun <- df_lenosos %>%
  filter(mun == params$id_municipio & cultivo %in% rk_lenosos_mun$cultivo) %>%
  select(cultivo, valor)
df_lenosos_C <- df_lenosos %>%
  filter(mun == "ES70" & cultivo %in% rk_lenosos_mun$cultivo) %>%
  select(cultivo, valor)
...
<div class="column-modulo-right" style="padding: 0px 23px;">
<div class="row">
<div class="column-3">

...
if(Variacion_Porcentual != "NA"){
  if(df_lv$tasa == 0){
    diferencia <- Num_Pobl_num - df$valor[which(df$mun == params$id_municipio & df$año == params$año-1)]
...
Num_Mujeres <- df_piramide %>% filter(mun == municipio_actual$municipio & gredad == "T" & sexo == "Mujeres" & año == params$año) %>% select(valor) %>%
  as.numeric %>% format(big.mark = ".", decimal.mark = ",")
Percent_Mujeres <- round(df_piramide %>% filter(mun == municipio_actual$municipio & gredad == "T" & sexo == "Mujeres" & año == params$año) %>% select(valor) / Num_Pobl_num *100, 1) %>%
  as.numeric %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
Num_Hombres <- df_piramide %>% filter(mun == municipio_actual$municipio & gredad == "T" & sexo == "Hombres" & año == params$año) %>% select(valor) %>%
  as.numeric %>% format(big.mark = ".", decimal.mark = ",")
Percent_Hombres <- round(df_piramide %>% filter(mun == municipio_actual$municipio & gredad == "T" & sexo == "Hombres" & año == params$año) %>% select(valor) / Num_Pobl_num *100, 1) %>%
  as.numeric %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
edad_media <- df_edadm %>% filter(mun == params$id_municipio & año == params$año) %>% select(valor) %>%
  as.numeric %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
TBM <- ((df_mortalidad %>% filter(mun == params$id_municipio & año == params$año) %>% select(valor) %>% as.numeric) / 100) %>% round(1) %>%
...
Crec_veg <- df_crec_veg %>% filter(año == params$año & mun == params$id_municipio) %>% select(valor) %>%
  as.numeric %>% format(big.mark = ".", decimal.mark = ",")
```
...
id_dep <- round(df_id_depen %>% filter(mun == params$id_municipio & año == params$año) %>% select(valor), 1) %>%
  as.numeric %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
emd_hombre <- df_emd_hombre %>% filter(mun == params$id_municipio & año == params$año) %>% select(valor) %>%
  as.numeric %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
peso_españoles <- df_nacionalidad_mun %>% filter(nacionalidad == "ESPAÑA") %>% select(porcentaje) %>% round(1) %>%
...
<div class="row" style="margin-top: 10px;">
  `r get_indicator_mod(
        "http://www.gobiernodecanarias.org/istac/estadisticas/demografia/",
        "Demografía"
        )`
...
<div class="row" style="margin-top: 10px;">
  `r get_indicator_mod(
        "https://www3.gobiernodecanarias.org/istac/statistical-visualizer/visualizer/data.html?resourceType=indicator&resourceId=POBLACION#visualization/table",
        "Población"
        )`
...
<div class="row" style="margin-top: 10px;">
  `r get_indicator_mod_first(
        "https://www3.gobiernodecanarias.org/istac/statistical-visualizer/visualizer/data.html?resourceType=indicator&resourceId=POBLACION_EDAD_MEDIA#visualization/table",
        "Población. Edad media")`
  `r get_indicator_mod_inter(
        "https://www3.gobiernodecanarias.org/istac/statistical-visualizer/visualizer/data.html?resourceType=indicator&resourceId=POBLACION_MUJERES#visualization/table",
        "Población. Mujeres")`
  `r get_indicator_mod_last(
        "https://www3.gobiernodecanarias.org/istac/statistical-visualizer/visualizer/data.html?resourceType=indicator&resourceId=POBLACION_HOMBRES#visualization/table",
        "Población. Hombres")`
...
<div class="row">
  `r get_indicator_mod_first(
        "http://www.gobiernodecanarias.org/istac/jaxi-istac/tabla.do?uripx=urn:uuid:304be2ca-3022-428a-99d0-b28bc4afbb9b&uripub=urn:uuid:253c609d-9d81-4266-986f-13ec9da19b28",
        "Población según sexos y grupos de edad")`
  `r get_indicator_mod_inter(
        "https://www3.gobiernodecanarias.org/istac/statistical-visualizer/visualizer/data.html?resourceType=indicator&resourceId=TASA_MORTALIDAD#visualization/table",
        "Tasa de mortalidad")`
  `r get_indicator_mod_inter(
        "http://www.gobiernodecanarias.org/istac/jaxi-istac/tabla.do?uripx=urn:uuid:3e369fce-cf23-4cb6-a9fa-10067b627496&uripub=urn:uuid:cc47adbb-2a55-4ddb-84cb-a540a6f34c50",
        "Crecimiento vegetativo")`
  `r get_indicator_mod_inter(
        "http://www.gobiernodecanarias.org/istac/jaxi-istac/tabla.do?uripx=urn:uuid:1c818013-8735-4989-baf1-8afd7c62bead&uripub=urn:uuid:98fecf86-426a-4eaf-aff4-ed6e039d2949",
        "Índice de dependencia")`
  `r get_indicator_mod_inter(
        "https://www3.gobiernodecanarias.org/istac/statistical-visualizer/visualizer/data.html?resourceType=indicator&resourceId=DEFUNCIONES_EDAD_MEDIA_MUJERES#visualization/table",
        "Edad media defunción mujer")`
  `r get_indicator_mod_inter(
        "https://www3.gobiernodecanarias.org/istac/statistical-visualizer/visualizer/data.html?resourceType=indicator&resourceId=DEFUNCIONES_EDAD_MEDIA_HOMBRES#visualization/table",
        "Edad media defunción hombre")`
  `r get_indicator_mod_inter(
        "http://www.gobiernodecanarias.org/istac/jaxi-istac/tabla.do?uripx=urn:uuid:3cd610d7-60f3-4dbc-be89-636bdbde9b16&uripub=urn:uuid:253c609d-9d81-4266-986f-13ec9da19b28",
        "Población según sexos y nacionalidades")`
  `r get_indicator_mod_last(
        "http://www.gobiernodecanarias.org/istac/jaxi-istac/tabla.do?uripx=urn:uuid:3863be4c-a35d-4523-9ed3-eae5100cae20&uripub=urn:uuid:253c609d-9d81-4266-986f-13ec9da19b28",
        "Población según sexos y lugares de nacimiento")`
...
if(max(df_nacimientos$año) == params$año-1 | max(df_mortalidad$año) == params$año-1 | max(df_crec_veg$año) == params$año-1 | max(df_id_depen$año) == params$año-1 | max(df_emd_mujer$año) == params$año-1 | max(df_emd_mujer$año) == params$año-1){
  cat(paste0(
    ...
    porc_lugar_Canarias, "%'></div><div class='progress-bar-blue3' style='width:", porc_lugar_RestoEspaña, "%'></div><div class='progress-bar-blue6' style='width:", porc_lugar_OtroPais, "%'></div></div><div class='progressbar-legend-container'><div class='progressbar-legend'><div class='progress-bar-blue1'></div><div class='sp-content'>Canarias<br/>",
    ...
    porc_lugar_Canarias_C, "%'></div><div class='progress-bar-grey3' style='width:", porc_lugar_RestoEspaña_C, "%'></div><div class='progress-bar-grey6' style='width:", porc_lugar_OtroPais_C, "%'></div></div><div class='progressbar-legend-container'><div class='progressbar-legend'><div class='progress-bar-grey1'></div><div class='sp-content'>Canarias<br/>",

...
# DF con todas las convocatorias de todos los tipos de elecciones y sus códigos.
...
# DF filtrando el tipo convocatoria incluyendo como debe aparecer la convocatoria escrita en la ficha y el orden para construir la serie.
convocatoria_municipales <- convocatoria %>% 
  filter(substring(code, 0,12) == "MUNICIPALES_") %>% 
  transform(url = paste0("url.", tolower(code)))
# DF incluyendo como debe aparecer la convocatoria escrita en la ficha y el orden para construir la serie.
...
# Convocatoria que se seleccionó.
...
```{r Gráfico espacio-temporal de la participación en el proceso electoral}
...
# DF de los valores de la variable principal en todos los procesos del mismo tipo y los territorios.
...
# DF con solo los municipios y el indicador tasa de participación.
df_u <- df_participacion %>% filter(substring(mun, 0,2) != "ES" & ind == "TASA_PARTICIPACION") %>% arrange(proceso) %>% select(!ind)

# DF de los valores de la variable principal en todos los municipios canarios en la convocatoria escogida.
related_mun <- df_u %>% filter(proceso == convocatoria_actual$code)
# Se aplica la función para seleccionar municipios relacionados en la convocatoria escogida.
...
# IF para que la serie que aparezca al escoger la primera convocatoria disponible no se filtre los proceso anteriores a esta, sino que se dejen todos los posteriores.
...
```{r Indicadores}
# DF de los valores de la variable para el municipio y convocatoria seleccionados.
...
# Icono que acompaña a la variación porcentual.
...
# Número y tasas de votos para los recuadros de los tipos de votos.
...
# IF para determinar el orden de las variables en el conjunto de datos (Todas las convocatorias no tienen la misma estructura).
...
```{r Resultados}
# DF de los resultados electorales para el municipio.
df_municipales_mun_ <- df_municipales %>% 
  filter(mun == params$id_municipio)

# DF de los resultados electorales solo de las candidaturas ordenadas de manera descendente.
...
# DF de las seis candidaturas más votadas.
...
# Se agrupan el resto de las candidaturas.
...
# Si no hay más de seis partidos en el municipio y convocatoria seleccionados esa modalidad se elimina.
...
# FOR para separar los nombres de los partidos políticos, sus siglas y sus porcentajes en varias líneas.
...
# Solo para las procesos electorales municipales.
# DF de los representantes elegidos de cada candidatura.
...
# FOR para separar los nombres de los partidos políticos y sus siglas en varias líneas.
...
# Función para los indicadores principales colocados después del gráfico espacio-temporal (icono representativo, valor/porcentaje y nombre).
...
# Función para los indicadores que están situados dentro de los recuadros de tipos de votos (modalidad, porcentaje y valor).
...
<h3 style="color: #999;margin: 0;float: right;margin-right: 20px;">Convocatoria</h3>
<h2 style="color: #666; margin: 0;">Resultados de las Elecciones (tipo)</h2>
<hr>
...
# Los tres indicadores principales, que se quedan en dos si no se tiene la variación porcentual ni se puede calcular.
if(Variacion == "NA"){
  cat(paste0(
...
<!-- Sólo para los procesos electorales municipales. -->
<div class="indicator-title-row">
...
# Pictograma para los representantes elegidos de cada candidatura.
# Primer FOR para cada candidatura y segundo FOR para el número de iconos por candidatura.
for (i in 1:nrow(df_representantes)) {
  cat(paste0(

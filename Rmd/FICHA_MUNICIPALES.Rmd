...
list.of.packages <- c("jsonlite", "ggplot2", "knitr", "data.table", "plotly", "plyr", "dplyr", "scales", "htmlwidgets", "sf", "fuzzyjoin", "xml2", "XML", "tidyverse", "tmap", "magrittr", "ggimage")
...
df_plot$name_partido <- ordered(df_plot$name_partido, levels = df_plot$name_partido[nrow(df_plot):1])
...
df_representantes_$name_partido_siglas <- ""

for(i in 1:nrow(df_representantes_)) {
  df_representantes_$name_partido_siglas[i] <- get_nombre_partido(df_representantes_$name_partido[i])
}
```
...
<div class="row highlight" style="width: 100% !important; margin-top: 10px; padding: 10px;">

```{r Representantes elegidos, results='asis'}
for (i in 1:nrow(df_representantes_)) {
  cat(paste0(
    "<div class='row'><div class='representantes'><p class='partido'>", df_representantes_$name_partido_siglas[i], "</p>"))
  for (j in 1:df_representantes_$repr_elegidos[i]) {
    cat(paste0(

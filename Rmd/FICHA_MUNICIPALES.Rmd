...
g_line <- ggplot(data = df, 
  ...
  scale_x_continuous(breaks = c(convocatoria_municipales$order[convocatoria_actual$order:1]), labels = c(convocatoria_municipales$convocatoria_formatted[convocatoria_actual$order:1])) +
...
df_municipales_mun_ <- df_municipales_mun %>% filter(substring(candidatura, 0,1) == "P") %>% arrange(valor) %>% select(medida, candidatura, valor)
...
df_otros <- df_municipales_mun %>% filter(substring(candidatura, 0,1) == "P" & candidatura %notin% df_plot$candidatura)
df_otros <- df_otros %>%
  spread(medida, valor) %>%
  ...
  col <- c("#D5EDFA", "#8CD2EA", "#2CBCE2", "#00A6DC", "#008BD0", "#0072A2", "#005980")
    ...
    col <- c("#8CD2EA", "#2CBCE2", "#00A6DC", "#008BD0", "#0072A2", "#005980")
      ...
      col <- c("#2CBCE2", "#00A6DC", "#008BD0", "#0072A2", "#005980")
        ...
        }else if(nrow(df_plot_) == 3){
          col <- c("#008BD0", "#0072A2", "#005980")
          }else if(nrow(df_plot_) == 2){
            col <- c("#0072A2", "#005980")
          }

g_municipales_mun <- ggplot(data = df_plot,
                         aes(y = candidatura, fill = candidatura, name = NULL, text = hovertext)) +
  geom_bar(stat = "identity", width = 0.6, aes(x = porcentaje)) +
  theme_minimal() +
...
df_representantes_ <- df_plot_ %>% filter(repr_elegidos > 0)%>% select(candidatura, repr_elegidos) %>% arrange(-repr_elegidos)

df_representantes <- df_representantes_ %>% rbind(c("", sum(df_representantes_$repr_elegidos)))
df_representantes <- df_representantes %>% left_join(df_partido, by = "candidatura") 
...
df_representantes <- df_representantes %>% transform(hovertext = paste0(
    "<b>", name_partido, " </b>",
    "<br>", trimws(format(repr_elegidos, big.mark = ".", decimal.mark = ",")), " representantes elegidos"
  )
)

df_representantes$hovertext[which(is.na(df_representantes$name_partido))] <- ""

if(nrow(df_representantes_) == 7){
  col <- c("#005980", "#0072A2", "#008BD0", "#00A6DC", "#2CBCE2", "#8CD2EA", "#D5EDFA", "white")
  }else if(nrow(df_representantes_) == 6){
    col <- c("#005980", "#0072A2", "#008BD0", "#00A6DC", "#2CBCE2", "#8CD2EA", "white")
    }else if(nrow(df_representantes_) == 5){
      col <- c("#005980", "#0072A2", "#008BD0", "#00A6DC", "#2CBCE2", "white")
      }else if(nrow(df_representantes_) == 4){
        col <- c("#005980", "#0072A2", "#008BD0", "#00A6DC", "white")
        }else if(nrow(df_representantes_) == 3){
          col <- c("#005980", "#0072A2", "#008BD0", "white")
          }else if(nrow(df_representantes_) == 2){
            col <- c("#005980", "#0072A2", "white")
          }

g_representantes <- plot_ly(df_representantes, labels = df_representantes$name_partido, values = df_representantes$repr_elegidos, type = 'pie', sort = TRUE, direction = "clockwise", 
                            hole = 0.6, textposition = "none", textinfo = "none", insidetextfont = list(color = "white"), hoverinfo = "text", text = df_representantes$hovertext,
                            rotation = 90, marker = list(colors = col, line = list(width = 0), alpha = 0.8)
                            )
...
<div class="piechart_P" style="height: 330px;">`r g_municipales_mun`</div>

<div class="tablechart" style="margin-top: 20px; margin-left: -5px;">

<div class="legend-vertical-container" style="margin-top: 10px;">
```{r leyenda Resultados, results='asis'}
for (i in 1:nrow(df_plot)) {
  cat(paste0(
    "<div class='progressbar-legend'><div class='progress-bar-blue",i,"'></div><div class='sp-content-b'>", df_plot[i,]$name_partido, "<br>(", format(round(df_plot[i,]$porcentaje, 1), big.mark = ".", decimal.mark = ",", nsmall = 1), "%)</div></div>"))
}
```
</div>
...
<div class="row highlight" style="width: 100% !important; margin-top: 10px;">
<div class="piechart_P" style="height: 280px; margin: 20px 10px -10px -20px;">`r g_representantes`</div>

<div class="tablechart" style="margin-top: 20px; margin-left: -5px;">

<div class="legend-vertical-container">
```{r leyenda Representantes, results='asis'}
for (i in 1:nrow(df_representantes_)) {
  cat(paste0(
    "<div class='progressbar-legend'><div class='progress-bar-blue",i,"'></div><div class='sp-content-b'>", df_representantes[i,]$name_partido, "<br>(", format(df_representantes[i,]$repr_elegidos, big.mark = ".", decimal.mark = ",", nsmall = 0), " representantes elegidos)</div></div>"))
}
```
</div>

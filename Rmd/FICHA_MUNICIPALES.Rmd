...
df_plot <- df_plot_ %>%
  arrange(desc(valor)) %>% top_n(5, valor) %>% select(-repr_elegidos)
...
if(nrow(df_plot) == 8){
  col <- c("#00405B", "#005980", "#0072A2", "#008BD0", "#00A6DC", "#2CBCE2", "#8CD2EA", "#D5EDFA", "")
  }else if(nrow(df_plot) == 7){
...
df_representantes <- df_plot_ %>% filter(repr_elegidos > 0)%>% select(candidatura, repr_elegidos) %>% arrange(-repr_elegidos)

df_representantes <- df_representantes %>% rbind(c("", sum(df_representantes$repr_elegidos)))
df_representantes <- df_representantes %>% left_join(df_partido, by = "candidatura") 
df_representantes$candidatura <- ordered(df_representantes$candidatura, levels = df_representantes$candidatura)
...
if(nrow(df_representantes) == 8){
  col <- c("#005980", "#0072A2", "#008BD0", "#00A6DC", "#2CBCE2", "#8CD2EA", "#D5EDFA", "white")
  }else if(nrow(df_representantes) == 7){
      col <- c("#005980", "#0072A2", "#008BD0", "#00A6DC", "#2CBCE2", "#8CD2EA", "white")
      }else if(nrow(df_representantes) == 6){
        col <- c("#005980", "#0072A2", "#008BD0", "#00A6DC", "#2CBCE2", "white")
        }else if(nrow(df_representantes) == 5){
          col <- c("#005980", "#0072A2", "#008BD0", "#00A6DC", "white")
        }

g_representantes <- plot_ly(df_representantes, labels = df_representantes$name_partido, values = df_representantes$repr_elegidos, type = 'pie', sort = TRUE, direction = "clockwise", hole = 0.6,
                           textposition = "none", textinfo = "none", insidetextfont = list(color = "white"), hoverinfo = "text", text = df_representantes$hovertext,
...
<p style="margin: -120px 20px 0; position: relative;">Representantes elegidos</p>
<div class="legend-vertical-container" style="margin-top: 6px;">
```{r leyenda Representantes, results='asis'}
for (i in 1:(nrow(df_representantes)-1)) {
  cat(paste0(
    "<div class='progressbar-legend'><div class='progress-bar-blue",i,"'></div><div class='sp-content-b'>", df_representantes[i,]$name_partido, " (", format(df_representantes[i,]$repr_elegidos, big.mark = ".", decimal.mark = ",", nsmall = 0), " representantes elegidos)</div></div>"))
}
```
</div>

...
mes <- data.frame(code = viajeros_lugar[["categories"]][["codes"]][[4]],
                  periodo = viajeros_lugar[["categories"]][["labels"]][[4]]) %>%
  filter(substring(code, 5,5) == "M") %>%
...
ADR <- df_ADR %>% filter(mes == mes_actual$code & mun == params$id_municipio) %>% select(valor) %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character()
```

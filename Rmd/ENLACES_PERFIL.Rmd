...
num_Canarias_total <- df_turistas %>% filter(mun == "ES70" & gredad == "TOTAL GRUPOS DE EDADES" & sexo == "AMBOS SEXOS" & pais == "TOTAL" & ano == params$ano) %>% 
  select(valor) %>% as.numeric
num_Isla_total <- df_turistas %>% filter(mun == municipio_actual$id_isla & gredad == "TOTAL GRUPOS DE EDADES" & sexo == "AMBOS SEXOS" & pais == "TOTAL" & ano == params$ano) %>%
  select(valor) %>% as.numeric
...
if(params$ano == 2018){
  df <- df %>% filter(ano %in% c(2018,2019,2020))
  }else{
...
g_line <- ggplot(data = df, aes(x = ano, y = valor, group = nombre, colour = nombre, text = tooltip)) +
  geom_line(size = 0.7) +
  theme_minimal() +
  ...
  scale_x_continuous(breaks = function(x){seq(from = min(df$ano), to = max(df$ano), by = max(1, as.integer((max(df$ano)-min(df$ano))/2)))}) +
...
g_line <- ggplotly(g_line, tooltip = "text") %>%
  ...
  layout(hoverlabel = "", hovermode = 'x unified',
         xaxis = list(autorange = FALSE,
                      range = c(max(min(df$ano), params$ano - 18), max(df$ano)),
...
```{r Número de turistas y su variación porcentual}
ind_turistas <- df %>% filter(id == params$id_municipio & ano == params$ano)

Num_Turistas_num <- ind_turistas$valor %>% as.numeric
if(is.na(Num_Turistas_num)) {stop("Error: indicador principal desconocido")}
Num_Turistas <- Num_Turistas_num %>% format(big.mark = ".", decimal.mark = ",")
Variacion_Porcentual <- ind_turistas$tasa %>% as.numeric %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
if(Variacion_Porcentual != "NA"){
  if(ind_turistas$tasa == 0){
    diferencia <- Num_Turistas_num - df$valor[which(df$id == params$id_municipio & df$ano == params$ano-1)]
...
Num_estancia_media <- df_turismo_estancia %>%
  filter(nivel == "TOTAL" & mun == municipio_actual$municipio & periodo == params$ano) %>% 
...
df_turismo_gedad <- df_turistas %>%
  filter(mun == municipio_actual$municipio & gredad != "TOTAL GRUPOS DE EDADES" & sexo == "AMBOS SEXOS" & pais == "TOTAL" & ano == params$ano) %>% 
  select(gredad, valor)
...
df_turismo_gedad_C <- df_turistas %>%
  filter(mun == "CANARIAS" & gredad != "TOTAL GRUPOS DE EDADES" & sexo == "AMBOS SEXOS" & pais == "TOTAL" & ano == params$ano) %>% 
  select(gredad, valor)
...
Percent_Mujeres <- round((
  df_turistas %>%
    filter(mun == municipio_actual$municipio & gredad == "TOTAL GRUPOS DE EDADES" & sexo == "Mujeres" & pais == "TOTAL" & ano == params$ano) %>%
    select(valor) %>% as.numeric) / Num_Turistas_num *100, 1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
Percent_Hombres <- round((
  df_turistas %>%
    filter(mun == municipio_actual$municipio & gredad == "TOTAL GRUPOS DE EDADES" & sexo == "Hombres" & pais == "TOTAL" & ano == params$ano) %>%
    select(valor) %>% as.numeric) / Num_Turistas_num *100, 1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
...
rk_turismo_pais_mun <- df_turistas %>%
  filter(gredad == "TOTAL GRUPOS DE EDADES"  & sexo == "AMBOS SEXOS" & pais != "TOTAL" & mun == municipio_actual$municipio & ano == params$ano) %>% arrange(valor) %>%
  select(pais, valor)
...
df_turismo_pais_mun <- df_turistas %>%
  filter(gredad == "TOTAL GRUPOS DE EDADES"  & sexo != "AMBOS SEXOS" & pais != "TOTAL" & mun == municipio_actual$municipio & ano == params$ano) %>%
...
df_turismo_aloj_mun <- df_turismo_sl_aloj %>%
  filter(tipo != "TOTAL" & sl == "TOTAL" & mun == municipio_actual$municipio & periodo == params$ano) %>% 
  select(tipo, valor)
df_turismo_aloj_C <- df_turismo_sl_aloj %>%
  filter(mun == "CANARIAS" & tipo != "TOTAL" & sl == "TOTAL" & periodo == params$ano) %>% 
  select(tipo, valor)
...
df_turismo_ingresos_C <- df_turismo_ingresos %>%
  filter(nivel != "TOTAL" & pais == "TOTAL" & mun == "CANARIAS" & periodo == params$ano) %>% 
  select(nivel, valor)
...
df_turismo_ingresos <- df_turismo_ingresos %>%
  filter(nivel != "TOTAL" & pais == "TOTAL" & mun == municipio_actual$municipio & periodo == params$ano) %>% 
  select(nivel, valor)
...
df_turismo_proposito <- df_turismo_proposito %>%
  filter(proposito != "TOTAL" & tipo == "TOTAL" & mun == municipio_actual$municipio & periodo == params$ano) %>% 
...
df_turismo_educacion_mun <- df_turismo_educacion %>%
  filter(estudios != "TOTAL" & pais == "TOTAL" & mun == municipio_actual$municipio & periodo == params$ano) %>% 
  select(estudios, valor)
df_turismo_educacion_C <- df_turismo_educacion %>%
  filter(estudios != "TOTAL" & pais == "TOTAL" & mun == "CANARIAS" & periodo == params$ano) %>% 
  select(estudios, valor)
...
df_turismo_motiv_C <- df_turismo_motiv %>%
  filter(motiv != "TOTAL" & tipo == "TOTAL" & mun == "CANARIAS" & periodo == params$ano) %>% 
  select(motiv, valor)
...
df_turismo_motiv <- df_turismo_motiv %>%
  filter(motiv != "TOTAL" & tipo == "TOTAL" & mun == municipio_actual$municipio & periodo == params$ano) %>% 
  select(motiv, valor)
...
df_links <- links %>%
  filter(Tematica == "perfil_turista") %>%
...
<div class="row" style="margin-top: 10px;">
```{r Enlaces Indicador principal-Indicadores principales-Ranking, results='asis'}
df_links_indicadores <- df_links %>%
  filter(Indicador %in% c("Indicador principal", "Indicadores principales", "Ranking") & !is.na(Enlace)) %>%
...
<h2 style="color: #005980; margin: 0;">`r Num_Turistas` turistas</h2>
<div class="linechart">
  `r g_line`
</div>
...
<div class="column-3" style="margin: 0;">
<div class="column" style="width: 100% !important; padding: 0;">
<div class="row highlight" style="width: 100% !important; padding: 3px; padding-bottom: 0;">
...
<div class="row highlight" style="width: 100% !important; padding-left: 0; margin-top: 8px;">
...
<div class="column-modulo-right">
<div class="highlight-modulo" style="padding-bottom: 5px;">
...
<div class="column highlight" style="width: 100% !important; margin-top: -30px; padding-top: 15px;">
  ...
  <div class='progressbar-legend'><div class='progress-bar-blue5' style='font-size: 11px;'></div><div class='sp-content'>`r df_turismo_aloj_plot$tipo[3]`</div></div>
  <div class='progressbar-legend'><div class='progress-bar-blue6' style='font-size: 11px;'></div><div class='sp-content'>`r df_turismo_aloj_plot$tipo[4]`</div></div>
  <div class='progressbar-legend'>
  <div class='progress-bar-C-b'>|</div><div class='sp-content-b'>Canarias</div></div>
  </div>
</div>

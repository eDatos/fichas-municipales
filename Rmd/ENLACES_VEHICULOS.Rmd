...
```{r Número de vehículos y su variación porcentual}
df_lv <- df_u %>% filter(mun == params$id_municipio & mes == mes_actual$code)

Num_Vehiculos_num <- df_lv %>% select(valor) %>% as.numeric
if(is.na(Num_Vehiculos_num)) {stop("Error: indicador principal desconocido")}
Num_Vehiculos <- df_lv %>% select(valor) %>% as.numeric %>% format(big.mark = ".", decimal.mark = ",")
Variacion_Porcentual <- df_lv %>% select(tasa) %>% as.numeric %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
if(Variacion_Porcentual != "NA"){
  if(df_lv$tasa == 0){
    diferencia <- Num_Vehiculos_num - df$valor[which(df$id == params$id_municipio & df$code == mes_tva$code)]
...
Percent_Tur_Parque <- round(Num_Tur_Parque_num / Num_Vehiculos_num * 100, 1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
df_vehiculos_mun <- df_vehiculos_mun %>%
  mutate(porcentaje = df_vehiculos_mun$valor / Num_Vehiculos_num * 100,
...
rk_combustibles_mun <- rk_combustibles_mun %>% mutate(porcentaje = rk_combustibles_mun$valor/ Num_Vehiculos_num * 100)
...
df_combustibles_mun <- df_combustibles_mun %>%
  mutate(porcentaje = df_combustibles_mun$valor / Num_Vehiculos_num * 100)
...
porc_gasolina_diesel_l <- round(porc_gasolina_diesel, 1) %>% as.numeric %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
porc_electrico_l <- round(porc_electrico, 1) %>% as.numeric %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
...
df_combustibles_otros <- df_combustibles_otros %>%
  mutate(porcentaje_barra = df_combustibles_otros$valor / sum(df_combustibles_otros$valor) * 100,
         porcentaje = df_combustibles_otros$valor / Num_Vehiculos_num * 100)
...
<div class="row" style="margin-top: 10px;">
  `r get_indicator_mod(
        "http://www.gobiernodecanarias.org/istac/estadisticas/sectorservicios/transporte/terrestre/",
...
<div class="row" style="margin-top: 10px;">
  `r get_indicator_mod(
        "https://www3.gobiernodecanarias.org/istac/statistical-visualizer/visualizer/data.html?resourceType=indicator&resourceId=PARQUE_VEHICULOS#visualization/table",
        "Parque de vehículos")`

<div class="column-modulo-right">
<div class="highlight-modulo">
<div class="linechart-placeholder">
<h2 style="color: #005980; margin: 0;">`r Num_Vehiculos` vehículos</h2>
<div class="linechart">
  `r g_line`
</div>
...
<div class="row" style="margin-top: 10px;">
  `r get_indicator_mod(
        "https://www3.gobiernodecanarias.org/istac/statistical-visualizer/visualizer/data.html?resourceType=indicator&resourceId=PARQUE_VEHICULOS_TURISMOS#visualization/table",
...
<div class="row">
  `r get_indicator_mod_first(
        "http://www.gobiernodecanarias.org/istac/jaxi-istac/tabla.do?uripx=urn:uuid:af270e8e-3b4b-4bfd-91ef-71231562bf9e&uripub=urn:uuid:172cc83a-4789-4f72-bf57-a4d0147c0656",
        "Parque de vehículos en circulación según tipos de vehículo y tipos de combustible")`
  `r get_indicator_mod_last(
        "http://www.gobiernodecanarias.org/istac/jaxi-istac/tabla.do?uripx=urn:uuid:a7b9825b-ad07-4610-aa79-0f9fa76afba7&uripub=urn:uuid:dd6e7a1d-8156-4fe7-be7e-e042bbb02873",
        "Vehículos matriculados según tipos de vehículo")`

...
df_viajeros_lugar <- df_viajeros %>% 
  filter(ind == "VIAJEROS_ALOJADOS" & periodo == mes_actual$code &
           lugar %notin% c("Total", "Otros países o territorios del mundo (excluida España)", "  Canarias", "  Residentes en España excepto Canarias")) %>%
...
df_viajeros_nacionalidad_mun <- df_mun %>%
  filter(lugar %in% c("España", "Mundo (excluida España)"))
...
peso_extranjera <- df_viajeros_nacionalidad_mun$porcentaje[which(df_viajeros_nacionalidad_mun$lugar == "Mundo (excluida España)")] %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character()
```
...
df_viajeros_canarios_mun <- df_mun %>%
  filter(lugar %in% c("  Canarias", "España", "Total"))
...
peso_Canarias_Tot <- df_viajeros_canarios_mun$porcentaje[which(df_viajeros_canarios_mun$lugar == "Total")] %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character()
```
...
df_hoteles <- df_hoteles %>% 
  transform(ind = recode(ind, "Establecimientos hoteleros abiertos" = "Establecimientos")) %>%
  transform(ind = recode(ind, "Plazas ofertadas" = "Plazas")) %>%
  transform(ind = recode(ind, "Habitaciones disponibles" = "Habitaciones/Apartamentos"))
...
df_extrahoteleros <- df_extrahoteleros %>%
  transform(ind = recode(ind, "Establecimientos abiertos" = "Establecimientos")) %>%
  transform(ind = recode(ind, "Plazas ofertadas" = "Plazas")) %>%
  transform(ind = recode(ind, "Apartamentos disponibles" = "Habitaciones/Apartamentos"))

...
peso_española <- (df_mun$valor[which(df_mun$lugar == "España")] / Num_Viajeros_num * 100) %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character()
peso_extranjera <- (df_mun$valor[which(df_mun$lugar == "Mundo (excluida España)")] / Num_Viajeros_num * 100) %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character()
```
...
num_viajeros_canarios_mun <- df_aloj_isla %>%
  filter(mun == params$id_municipio & periodo == mes_actual$code & lugar == "Canarias") %>% select(valor) %>% as.numeric()
...
df_aloj_isla_mun <- df_aloj_isla %>%
  filter(mun == params$id_municipio & periodo == mes_actual$code & lugar %notin% c("España (excluida Canarias)", "Canarias")) %>% select(lugar, valor)
df_aloj_isla_C <- df_aloj_isla %>%
filter(mun == "ES70" & periodo == mes_actual$code & lugar %notin% c("España (excluida Canarias)", "Canarias")) %>% select(lugar, valor)
...
```{r Indicadores Islas}
peso_Canarias_Esp <- (num_viajeros_canarios_mun / df_mun$valor[which(df_mun$lugar == "España")] * 100) %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character()
peso_Canarias_Tot <- (num_viajeros_canarios_mun / df_mun$valor[which(df_mun$lugar == "Total")] * 100) %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character()
```
...
extrahoteleros <- fromJSON(params$url.apartamentos)
...
Num_Habitaciones_H <- df_establecimientos_mun$valor_hoteles[which(df_establecimientos_mun$ind == "Habitaciones")] %>% format(big.mark = ".", decimal.mark = ",")
...
Num_Habitaciones_A <- df_establecimientos_mun$valor_extrahoteleros[which(df_establecimientos_mun$ind == "Habitaciones")] %>% format(big.mark = ".", decimal.mark = ",")
```
...
if(Variacion_Porcentual == "NA"){
  cat(paste0(
    "<div class='column-3-ind'>", get_indicator2('Estancia<br/>media', Estancia_media, './img/cama.png'), "</div><div class='column-3-ind'>", get_indicator2('Tasa de<br/>ocupación', Tasa_ocupacion,'./img/percent-blue.png'), "</div><div class='column-3-ind'>", get_indicator2('Tarifa media<br/>diaria', ADR, './img/euro.png'), "</div>"))
}else{
  cat(paste0(
    "<div class='column-4'>", get_indicator2('Tasa de<br/>variación', paste0(Variacion_Porcentual, '%'), Imagen_Variacion), "</div><div class='column-4'>", get_indicator2('Estancia<br/>media', Estancia_media, './img/cama.png'), "</div><div class='column-4'>", get_indicator2('Tasa de<br/>ocupación', Tasa_ocupacion,'./img/percent-blue.png'), "</div><div class='column-4'>", get_indicator2('Tarifa media<br/>diaria', ADR, './img/euro.png'), "</div>"))
}
```
</div>

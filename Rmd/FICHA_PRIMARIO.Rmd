...
df_herbaceos$name_cultivo <- ""
for(i in 1:nrow(df_herbaceos)) {
  df_herbaceos$name_cultivo[i] <- get_nombre(paste0(df_herbaceos$cultivo[i]))
}
...
rk_herbaceos_mun <- df_herbaceos %>%
  ...
  arrange(valor) %>%
  select(cultivo, name_cultivo, valor)
...
df_herbaceos_plot <- df_herbaceos_plot %>%
  left_join(rk_herbaceos_mun, by = c("cultivo", "name_cultivo", "valor")) %>% 
  arrange(desc(order))
...
df_leñosos$name_cultivo <- ""
for(i in 1:nrow(df_leñosos)) {
  df_leñosos$name_cultivo[i] <- get_nombre(paste0(df_leñosos$cultivo[i]))
}
...
df_leñosos_plot <- df_leñosos_plot %>% 
  left_join(rk_leñosos_mun, by = c("cultivo", "name_cultivo", "valor")) %>% 
  arrange(desc(order))

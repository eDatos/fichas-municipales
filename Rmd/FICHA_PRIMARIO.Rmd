...
params:
  ...
  url.cultivada: https://datos.canarias.es/api/estadisticas/indicators/v1.0/indicatorsSystems/C00067A/indicatorsInstances/2b88193e-29c3-44ae-91dd-c17bbe90124a/data?representation=MEASURE%5BABSOLUTE%7CANNUAL_PERCENTAGE_RATE%5D&granularity=GEOGRAPHICAL%5BMUNICIPALITIES%5D
  ...
  url.exp_especies: https://datos.canarias.es/api/estadisticas/statistical-resources/v1.0/datasets/ISTAC/E01008B_000002/1.0
  url.cab_especies: https://datos.canarias.es/api/estadisticas/statistical-resources/v1.0/datasets/ISTAC/E01008B_000001/1.0
output:
  html_document:
    df_print: paged
...
df_u <- data.frame(Municipio = rep(names(data_u[["dimension"]][["GEOGRAPHICAL"]][["representation"]][["index"]]),
                   ...
                   Año = as.integer(names(data_u[["dimension"]][["TIME"]][["representation"]][["index"]])), 
...
df_lv <- data.frame(
  municipio = rep(names(ind_pobl[["dimension"]][["GEOGRAPHICAL"]][["representation"]][["index"]]),
                  each=ind_pobl[["dimension"]][["TIME"]][["representation"]][["size"]] * ind_pobl[["dimension"]][["MEASURE"]][["representation"]][["size"]]),
  ...
  valor = round(as.numeric(ind_pobl[["observation"]]), 1))
df_lv$año <- as.integer(df_lv$año)

df_u <- df_u %>% 
  left_join((df_lv %>% filter(medida == "ANNUAL_PERCENTAGE_RATE") %>% select(Municipio = municipio, Año = año, Tasa = valor)), by = c("Municipio", "Año"))
...
df <- related_mun %>%
  ...
  select(Año, Valor, Municipio = municipio, Tasa) %>%
...
df_lv <- df_lv %>% filter(año == params$año & municipio == params$id_municipio)
...
Variacion_Porcentual_Pobl <- format(df_lv[df_lv$medida == 'ANNUAL_PERCENTAGE_RATE', ]$valor, big.mark = ".", decimal.mark = ",", nsmall = 1)
```

```{r Rankings}
df_ranking <- data.frame(id = rep(names(data_u[["dimension"]][["GEOGRAPHICAL"]][["representation"]][["index"]]),
                               each=data_u[["dimension"]][["TIME"]][["representation"]][["size"]]), 
...
percent_canarias <- format(round(df_ranking[which(df_ranking$id==params$id_municipio),]$valor / Num_Pobl_Canarias * 100, 1), big.mark = ".", decimal.mark = ",", nsmall = 1)
...
percent_isla <- format(round(df_ranking_isla[which(df_ranking_isla$id==params$id_municipio),]$valor / Num_Pobl_Isla * 100, 1), big.mark = ".", decimal.mark = ",", nsmall = 1)
...
df_matr_tipos$periodo <- factor(df_matr_tipos$periodo, levels=data_matr_tipos[["categories"]][["codes"]][[3]], labels=paste0(substring(data_matr_tipos[["categories"]][["codes"]][[3]], 0,4), "-",
                                                                                                                             substring(data_matr_tipos[["categories"]][["codes"]][[3]], 5,7)))
...
<div class="piramide" style="height: 360px">
  `r g_matr_tipos`
</div>

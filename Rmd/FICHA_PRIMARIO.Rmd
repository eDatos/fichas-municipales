...
params:
  ...
  url.agr_lenosos: http://www.gobiernodecanarias.org/istac/jaxi-istac/tabla.do?accion=jsonMtd&uuidConsulta=bba81127-8204-4fbb-8b37-4872748439df
...
colnames(df_herbaceos) <- c('mun', 'cultivo', 'ano', 'sistema', 'valor')
...
df_herbaceos$ano <- as.numeric(df_herbaceos$ano)
...
sup_herbaceos_num <- df_herbaceos %>%
  filter(mun == params$id_municipio & ano == params$ano & cultivo == "TOTAL CULTIVOS HERBÁCEOS" & sistema == "TOTAL (SECANO+REGADÍO)") %>% select(valor) %>% as.numeric
sup_herbaceos <- sup_herbaceos_num %>% format(big.mark = ".", decimal.mark = ",")
sup_herbaceos_C <- df_herbaceos %>%
  filter(mun == "ES70" & ano == params$ano & cultivo == "TOTAL CULTIVOS HERBÁCEOS" & sistema == "TOTAL (SECANO+REGADÍO)") %>% select(valor) %>% as.numeric
...
df_herbaceos <- df_herbaceos %>% 
  filter(substring(cultivo, 0,1) == " " & substring(cultivo, 0,4) != " Otr" & sistema == "TOTAL (SECANO+REGADÍO)" & ano == params$ano) %>% 
...
```{r Barras leñosos}
data_lenosos <- fromJSON(params$url.agr_lenosos)
df_lenosos <- cbind(data.frame(do.call('rbind', data_lenosos[["data"]][["dimCodes"]])), data_lenosos[["data"]][["Valor"]])
colnames(df_lenosos) <- c('mun', 'cultivo', 'ano', 'sistema', 'valor')
lista_cultivos <- c(data_lenosos[["categories"]][["labels"]][[2]][1:52], " Viveros")
df_lenosos$cultivo <- factor(df_lenosos$cultivo, levels=data_lenosos[["categories"]][["codes"]][[2]], labels=lista_cultivos)
df_lenosos$ano <- as.numeric(df_lenosos$ano)
df_lenosos$sistema <- factor(df_lenosos$sistema, levels=data_lenosos[["categories"]][["codes"]][[4]], labels=data_lenosos[["categories"]][["labels"]][[4]])
df_lenosos$valor <- as.numeric(df_lenosos$valor)
df_lenosos$mun <- recode_id_Frontera(df_lenosos, 'mun')

df_lenosos_mun <- df_lenosos %>%
  filter(mun == params$id_municipio & ano == params$ano & sistema == "TOTAL EN PLANTACIÓN REGULAR (SECANO+REGADÍO)")

sup_lenosos_num <- df_lenosos_mun %>%
  filter(cultivo == "TOTAL CULTIVOS LEÑOSOS") %>% select(valor) %>% as.numeric
sup_lenosos <- sup_lenosos_num %>% format(big.mark = ".", decimal.mark = ",")
sup_lenosos_C <- df_lenosos %>%
  filter(mun == "ES70" & cultivo == "TOTAL CULTIVOS LEÑOSOS" & ano == params$ano & sistema == "TOTAL EN PLANTACIÓN REGULAR (SECANO+REGADÍO)") %>% select(valor) %>% as.numeric

percent_lenosos <- round(sup_lenosos_num / superficie_cultivada_num * 100, 1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)
percent_lenosos_C <- round(sup_lenosos_C / sup_Canarias_total * 100, 1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1)

df_lenosos_mun <- df_lenosos_mun %>%
  filter(substring(cultivo, 0,1) == " " & substring(cultivo, 0,4) != " Otr") %>%
  transform(cultivo = trimws(cultivo))

rk_lenosos_mun <- df_lenosos_mun %>%
  top_n(8, valor) %>% 
  arrange(cultivo) %>%
  arrange(desc(valor)) %>%
  select(cultivo, valor)

if(length(rk_lenosos_mun$cultivo) > 8){
  rk_lenosos_mun <- rk_lenosos_mun[1:8, ]
}
rk_lenosos_mun$cultivo <- ordered(rk_lenosos_mun$cultivo, levels = rk_lenosos_mun$cultivo[8:1])

df_lenosos_mun <- df_lenosos_mun %>%
  filter(cultivo %in% rk_lenosos_mun$cultivo) %>%
  select(cultivo, valor)
df_lenosos_C <- df_lenosos %>%
  transform(cultivo = trimws(cultivo)) %>%
  filter(mun == "ES70" & cultivo %in% rk_lenosos_mun$cultivo & ano == params$ano & sistema == "TOTAL EN PLANTACIÓN REGULAR (SECANO+REGADÍO)") %>%
  select(cultivo, valor)
df_lenosos_mun <- df_lenosos_mun %>%
  mutate(porcentaje = df_lenosos_mun$valor / sup_lenosos_num * 100,
         valor_Canarias = df_lenosos_C$valor,
         porcentaje_Canarias = df_lenosos_C$valor / sup_lenosos_C * 100)

df_lenosos_mun$name_cultivo <- ""
for(i in 1:nrow(df_lenosos_mun)) {
  df_lenosos_mun$name_cultivo[i] <- get_nombre(paste0(df_lenosos_mun$cultivo[i]))
}

df_lenosos_plot <- df_lenosos_mun %>%
  arrange(desc(name_cultivo)) %>%
  arrange(desc(valor)) %>% 
  mutate(label = "|")
df_lenosos_plot$cultivo <- ordered(df_lenosos_plot$cultivo, levels = rk_lenosos_mun$cultivo[8:1])
df_lenosos_plot <- df_lenosos_plot %>% 
  left_join(rk_lenosos_mun, by = c("cultivo", "valor")) %>% 
  arrange(desc(cultivo))

df_lenosos_plot <- df_lenosos_plot %>% transform(hovertext = paste0(
...
if(nrow(df_lenosos_plot)/2 == 8){
  col <- c("#D5EDFA", "#8CD2EA", "#2CBCE2", "#00A6DC", "#008BD0", "#0072A2", "#005980", "#00405B")
  size_text <- 6
  }else if(nrow(df_lenosos_plot)/2 == 7){
    col <- c("#D5EDFA", "#8CD2EA", "#2CBCE2", "#00A6DC", "#008BD0", "#0072A2", "#005980")
    size_text <- 7
    }else if(nrow(df_lenosos_plot)/2 == 6){
      col <- c("#8CD2EA", "#2CBCE2", "#00A6DC", "#008BD0", "#0072A2", "#005980")
      size_text <- 8
      }else if(nrow(df_lenosos_plot)/2 == 5){
        col <- c("#8CD2EA", "#2CBCE2", "#008BD0", "#0072A2", "#005980")
        size_text <- 9
        }else if(nrow(df_lenosos_plot)/2 == 4){
          col <- c("#8CD2EA", "#2CBCE2", "#008BD0", "#005980")
          size_text <- 10
          }else if(nrow(df_lenosos_plot)/2 == 3){
            col <- c("#8CD2EA", "#008BD0", "#005980")
            size_text <- 15
            }else if(nrow(df_lenosos_plot)/2 == 2){
              col <- c("#8CD2EA", "#005980")
              size_text <- 20
            }

g_lenosos <- ggplot(data = df_lenosos_plot, aes(y = cultivo, fill = cultivo, label = label, name = NULL, text = hovertext)) +
  geom_bar(stat = "identity", alpha = 0.8, width = 0.6, aes(x = porcentaje)) +
  geom_text(size = size_text, color = "#59656E", aes(x = porcentaje_Canarias, text = "")) +
  theme_minimal() +
  labs(title = "", x = NULL, y = NULL, colour = " ") +
  ...
  theme(axis.text.x = element_text(),
        ...
        legend.position = "none") +
  scale_x_continuous(labels = function(x){paste0(format(x, big.mark = '.', decimal.mark = ","), '%')}) +
  scale_y_discrete(labels = df_lenosos_plot$name_cultivo[(nrow(df_lenosos_plot)/2):1])

g_lenosos <- ggplotly(g_lenosos, tooltip = c("text")) %>%
  layout(hoverlabel = "", hovermode = 'y unified', margin = list(l = 0, r = 0)) %>%
  config(displaylogo = FALSE,  modeBarButtons = list(list("zoom2d"),list("pan2d"),list("resetScale2d"),list("toImage"))) %>%
  style(hoverinfo = "skip", traces = c((nrow(df_lenosos_plot)/2+1):24))

df_viveros <- df_lenosos %>%
  filter(substring(mun, 0,2) != "ES" & cultivo == " Viveros" & sistema == "TOTAL EN PLANTACIÓN REGULAR (SECANO+REGADÍO)" & valor != 0)
df_viveros$mun <- factor(df_viveros$mun)
```
...
<div class="column highlight">
<div class="row">
<div class='indicator5'><div class='indicator-content'><p class='value'>`r sup_lenosos` ha</p><hr class='separator'><p class='label'>Superficie cultivada</p></div></div>
</div>

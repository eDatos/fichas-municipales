...
g_line <- ggplot(data = df, 
  ...
  scale_x_continuous(breaks = function(x){seq(from = max(max(df$año) - 8, min(df$año)), to = max(df$año), by = as.integer((max(df$año)-max(max(df$año) - 18, min(df$año)))/3))}) +
...
superficie_cultivada <- format(superficie_cultivada_num, big.mark = ".", decimal.mark = ",")
Imagen_Variacion <- ifelse(signo(df_lv$tasa) == "más", './img/up.png', './img/down.png')
Variacion_Porcentual <- format(df_lv$tasa, big.mark = ".", decimal.mark = ",", nsmall = 1)
```
...
sup_secano <- sup_secano_num %>% format(big.mark = ".", decimal.mark = ",")
...
sup_regadio <- sup_regadio_num %>% format(big.mark = ".", decimal.mark = ",")
...
```{r Barras países por sexo, eval=FALSE}
...
```{r Tipos de alojamiento, eval=FALSE}
...
```{r Niveles de ingresos, eval=FALSE}
...
```{r Propósito principal, eval=FALSE}
...
```{r Motivaciones principales, eval=FALSE}

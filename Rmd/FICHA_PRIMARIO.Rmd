...
leyenda_poblacion <- df %>% filter(año == params$año) %>%
  mutate(valor = ifelse(is.na(valor), " -", paste0(format(valor, big.mark = ".", decimal.mark = ",", nsmall = 1), " m<sup>2</sup>")),
...
df <- df %>% mutate(
    tooltip = ifelse(año == params$año,
                     sprintf(leyenda.content, nombre, ifelse(is.na(valor), " -", paste0(format(valor, big.mark = ".", decimal.mark = ",", nsmall = 1), ' m<sup>2</sup>')),
                             ifelse(is.na(tasa), " -", paste0(format(tasa, big.mark = ".", decimal.mark = ",", nsmall = 1), "%"))),
                     sprintf(leyenda.content, nombre, paste0(ifelse(is.na(valor), " -", format(valor, big.mark = ".", decimal.mark = ",", nsmall = 1)), ' m<sup>2</sup> (',año,')'),
                             ifelse(is.na(tasa), " -", paste0(format(tasa, big.mark = ".", decimal.mark = ",", nsmall = 1), "%")))
    )
) 
...
rk_herbaceos_mun <- rk_herbaceos_mun %>%
  mutate(order = as.integer(order(rk_herbaceos_mun$valor, decreasing = FALSE)))
...
df_herbaceos_plot <- df_herbaceos_plot %>% transform(hovertext = paste0(
  ...
  " m<sup>2</sup> (", trimws(format(round(porcentaje, 1), big.mark = ".", decimal.mark = ",", nsmall = 1)), "%)",
  "<br>Canarias: ", trimws(format(valor_Canarias, big.mark = ".", decimal.mark = ",", nsmall = 1)),
  " m<sup>2</sup> (", trimws(format(round(porcentaje_Canarias, 1), big.mark = ".", decimal.mark = ",", nsmall = 1)), "%)"
  )
)
...
g_herbaceos <- ggplot(data = df_herbaceos_plot, aes(y = reorder(cultivo, order), fill = cultivo, label = label, name = NULL, text = hovertext)) +
...
rk_leñosos_mun <- rk_leñosos_mun %>% 
  mutate(order = as.integer(order(rk_leñosos_mun$valor, decreasing = FALSE)))
...
g_leñosos <- ggplot(data = df_leñosos_plot, aes(y = reorder(cultivo, order), fill = cultivo, label = label, name = NULL, text = hovertext)) +
  geom_bar(stat = "identity", alpha = 0.8, width = 0.6, aes(x = porcentaje)) +
  geom_text(size = 7, color = "#59656E", aes(x = porcentaje_Canarias, text = "")) +
  theme_minimal() +
  labs(title = "", x = NULL, y = NULL, colour = " ") +
...
if(nrow(rk_exp_especies_mun) == 5){
  bar_size <- 5
  }else if(nrow(rk_exp_especies_mun) == 4){
    bar_size <- 6
    }else if(nrow(rk_exp_especies_mun) == 3){
      bar_size <- 8
      }else if(nrow(rk_exp_especies_mun) == 2){
        bar_size <- 10
      }

rk_exp_especies_mun$especie <- ordered(rk_exp_especies_mun$especie, levels = rk_exp_especies_mun$especie)

rk_exp_especies_mun <- rk_exp_especies_mun %>%
  mutate(order = as.integer(order(rk_exp_especies_mun$valor, decreasing = FALSE)))
...
df_exp_especies_plot <- df_exp_especies_plot %>%
  left_join(rk_exp_especies_mun, by = c("especie", "valor")) %>% 
  arrange(desc(order))
...
g_exp_especies <- ggplot(data = df_exp_especies_plot, aes(y = reorder(especie, order), fill = especie, label = label, name = NULL, text = hovertext)) +
  geom_bar(stat = "identity", alpha = 0.8, width = 0.6, aes(x = porcentaje)) +
  geom_text(size = bar_size, color = "#59656E", aes(x = porcentaje_Canarias, text = "")) +
  theme_minimal() +
  labs(title = "", x = NULL, y = NULL, colour = " ") +
  guides(fill = FALSE) +
  scale_fill_manual(values = col) +
...
df_cab_especies_mun <- df_cab_especies %>%
  filter(mun == municipio_actual$id & año == params$año)
...
if(Variacion_Porcentual == "NA"){
  cat(paste0(
    "<div class='column-3-ind'>", get_indicator2(paste0('Secano</br></p><p class="label" style="color: #999;">', paste0(percent_secano, '%')), paste0(sup_secano, " m<sup>2</sup>"), './img/secano.png'), "</div><div class='column-3-ind'>", get_indicator2(paste0('Regadío</br></p><p class="label" style="color: #999;">', paste0(percent_regadio, '%')), paste0(sup_regadio, " m<sup>2</sup>"), './img/regadio.png'), "</div><div class='column-3-ind'>", get_indicator2("Cultivada<br>vs total", percent_sup_cultivada, "./img/percent-blue.png"), "</div>"))
}else{
  cat(paste0(
    "<div class='column-4'>", get_indicator2('Tasa de</br>variación', paste0(Variacion_Porcentual, '%'), Imagen_Variacion), "</div><div class='column-4'>", get_indicator2(paste0('Secano</br></p><p class="label" style="color: #999;">', paste0(percent_secano, '%')), paste0(sup_secano, " m<sup>2</sup>"), './img/secano.png'), "</div><div class='column-4'>", get_indicator2(paste0('Regadío</br></p><p class="label" style="color: #999;">', paste0(percent_regadio, '%')), paste0(sup_regadio, " m<sup>2</sup>"), './img/regadio.png'), "</div><div class='column-4'>", get_indicator2("Cultivada<br>vs total", percent_sup_cultivada, "./img/percent-blue.png"), "</div>"))
}
```
</div>
...
<div class="column highlight" style="padding: 26px 0;">
<div class="row" style="margin-bottom: 27px;">
  ...
  <div class="column-2">`r get_indicator3("Bovinos", bovinos_num, "./img/balanza.png")`</div> 
  <div class="column-2">`r get_indicator3("Caprinos", caprinos_num, "./img/balanza.png")`</div> 
</div>
<div class="row">
  ...
  <div class="column-2">`r get_indicator3("Ovinos", ovinos_num, "./img/balanza.png")`</div> 
  <div class="column-2">`r get_indicator3("Porcinos", porcinos_num, "./img/balanza.png")`</div> 
</div>

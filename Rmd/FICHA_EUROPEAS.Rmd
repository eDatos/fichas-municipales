...
df_europeas_C <- df_europeas %>% 
  spread(medida, valor) %>% filter(!is.na(VOTOS_VALIDOS_CANDIDATURA)) %>% 
  select(mun, candidatura, valor_C = VOTOS_VALIDOS_CANDIDATURA, porcentaje_C = RATIO_VOTOS_CANDIDATURA) %>% 
  filter(mun == "ES70") %>% select(!mun)

df_plot_C_ <- df_europeas_C %>% filter(substring(candidatura, 0,1) == "P" & candidatura %in% df_plot$candidatura) %>% arrange(valor_C)

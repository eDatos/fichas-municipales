...
df_plot_C_ <- df_europeas_C %>% filter(substring(candidatura, 0,1) == "P" & candidatura %in% df_plot$candidatura) %>% arrange(valor) %>% select(medida, candidatura, valor)
...
df_plot <- df_plot %>% transform(hovertext = paste0(
  ...
  "<br>Canarias: ", trimws(format(valor_C, big.mark = ".", decimal.mark = ",")), " votos",
...
df_plot$name_partido <- ordered(df_plot$name_partido, levels = df_plot$name_partido[nrow(df_plot_C):1])

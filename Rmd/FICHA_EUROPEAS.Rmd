...
```{r, eval=FALSE}
data_europeas_2019 <- fromJSON(params$url.europeas_2019)
data_europeas_2016 <- fromJSON(params$url.europeas_2014)
data_europeas_2008 <- fromJSON(params$url.europeas_2009)
data_europeas_2004 <- fromJSON(params$url.europeas_2004)
```
...
data_electores <- fromJSON(params$url.europeas_electores)
...
convocatoria <- convocatoria %>% 
  transform(convocatoria_formatted = paste0(unlist(lapply(strsplit(convocatoria, " "), '[[', 2))),
            url = paste0("url.europeas_", substring(code, 0,6)))
...
df_europeas <- df_europeas %>% transform(mun = if_else(mun == "38013_2007",  "38013", mun))
```
...
```{r, eval=FALSE}
df_europeas <- data.frame(
  medida = rep(data_europeas_201911[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[1]][["code"]],
               each = length(data_europeas_201911[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[2]][["code"]]) *
                      length(data_europeas_201911[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[3]][["code"]])),
  mun = rep(data_europeas_201911[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[2]][["code"]],
            each = length(data_europeas_201911[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[3]][["code"]])),
  candidatura = data_europeas_201911[["data"]][["dimensions"]][["dimension"]][["representations"]][["representation"]][[3]][["code"]],
  valor = unlist(strsplit(data_europeas_201911[["data"]][["observations"]], split = " | ", fixed = TRUE)))
df_europeas$valor <- as.numeric(df_europeas$valor)
df_europeas <- df_europeas %>% transform(mun = if_else(mun == "38013_2007",  "38013", mun))

df_partido <- data.frame(
  candidatura = data_europeas_201911[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["value"]][[3]][["id"]],
  name_partido = vector(mode = "character", length = data_europeas_201911[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["total"]][3]))
for(i in 1:data_europeas_201911[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["total"]][3]) {
  df_partido$name_partido[i] <- data_europeas_201911[["metadata"]][["dimensions"]][["dimension"]][["dimensionValues"]][["value"]][[3]][["name"]][["text"]][[i]][["value"]]
}
```


```{r Resultados}
df_europeas_mun <- df_europeas %>% 
  left_join(df_partido, by = "candidatura") %>%
  filter(mun == params$id_municipio)

partido_mas_votado <- df_europeas_mun %>% filter(medida == "VOTOS_VALIDOS_CANDIDATURA" & substring(candidatura, 0,1) == "P") %>% top_n(1, valor) %>%
  select(name_partido) %>% as.character()
df_plot_ <- df_europeas_mun %>% filter(substring(candidatura, 0,1) == "G") %>% arrange(valor) %>% select(c(medida, name_partido, valor))
...
g_europeas_mun <- plot_ly(df_plot, labels = df_plot$name_partido, values = df_plot$porcentaje, type = 'pie', sort = TRUE, direction = "clockwise", hole = 0.6,
...
partido_mas_votado_C <- df_europeas_C %>%
  filter(medida == "VOTOS_VALIDOS_CANDIDATURA" & substring(candidatura, 0,1) == "P") %>% top_n(1, valor) %>% select(name_partido) %>% as.character()
df_plot_C_ <- df_europeas_C %>% filter(substring(candidatura, 0,1) == "G") %>% arrange(valor) %>% select(c(medida, name_partido, valor))
...
g_europeas_C <- plot_ly(df_plot_C, labels = df_plot_C$name_partido, values = df_plot_C$porcentaje, type = 'pie', sort = TRUE, direction = "clockwise", hole = 0.6,
...
g_europeas_C <- ggplotly(g_europeas_C, tooltip = c("text")) %>%
  layout(showlegend = FALSE) %>%
  config(displaylogo = FALSE, modeBarButtons = list(list("zoom2d"),list("pan2d"),list("resetScale2d"),list("toImage")))
```
...
<h2 style="color: #666; margin: 0;">Resultados electorales al Parlamento Europeo</h2>
<hr>

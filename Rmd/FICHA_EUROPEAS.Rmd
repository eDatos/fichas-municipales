...
convocatoria <- convocatoria %>% 
  transform(convocatoria_formatted = paste0(unlist(lapply(strsplit(convocatoria, " "), '[[', 2))),
            url = paste0("url.europeas_", substring(code, 0,4)))
...
df_u_ <- df_u %>% filter(substring(Municipio, 0,2) != "ES" & Residencia == "000000") %>% arrange(Convocatoria) %>% select(!Residencia)
...
Num_Electores <- df_lv %>% filter(Residencia == "000000") %>% select(Valor) %>% as.numeric() %>% format(big.mark = ".", decimal.mark = ",")
...
Num_Residentes_Canarias <- df_lv %>% filter(Residencia == "000002") %>% select(Valor) %>% as.numeric() %>% format(big.mark = ".", decimal.mark = ",")
Num_Residentes_Extranjero <- df_lv %>% filter(Residencia == "000003") %>% select(Valor) %>% as.numeric() %>% format(big.mark = ".", decimal.mark = ",")
Num_Residentes_Canarias_UE <- df_lv %>% filter(Residencia == "000004") %>% select(Valor) %>% as.numeric() %>% format(big.mark = ".", decimal.mark = ",")
```

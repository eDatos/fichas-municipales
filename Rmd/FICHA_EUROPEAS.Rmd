...
```{r Convocatoria}
data_participacion <- fromJSON(params$url.europeas_participacion)
...
convocatoria_actual <- convocatoria_europeas %>% filter(substring(code, 20,23) == params$convocatoria)
...
min_code <- min(convocatoria_europeas$code)
...
g_line <- ggplot(data = df, 
  ...
  scale_x_continuous(breaks = c(convocatoria_europeas$order[convocatoria_actual$order:1]), labels = c(convocatoria_europeas$convocatoria_formatted[convocatoria_actual$order:1])) +
...
if(params$convocatoria %in% c(2019, 2004)){
...
df_plot_ <- df_europeas_mun %>% filter(substring(candidatura, 0,1) == "P") %>% arrange(valor) %>% select(medida, candidatura, valor)
df_plot <- df_plot_ %>%
...
df_otros <- df_europeas_mun %>% filter(substring(candidatura, 0,1) == "P" & candidatura %notin% df_plot$candidatura)
df_otros <- df_otros %>%
  spread(medida, valor) %>%
  select(candidatura, valor = VOTOS_VALIDOS_CANDIDATURA, porcentaje = RATIO_VOTOS_CANDIDATURA)
...
df_europeas_P <- df_europeas %>% 
  left_join(df_partido, by = "candidatura") %>%
  filter(mun == municipio_actual$id_provincia)

df_plot_P_ <- df_europeas_P %>% filter(substring(candidatura, 0,1) == "P" & candidatura %in% df_plot$candidatura) %>% arrange(valor) %>% select(medida, candidatura, valor)
...
df_otros <- df_europeas_P %>% filter(substring(candidatura, 0,1) == "P" & candidatura %notin% df_plot$candidatura)
df_otros <- df_otros %>%
  spread(medida, valor) %>%
  select(candidatura, valor_P = VOTOS_VALIDOS_CANDIDATURA, porcentaje_P = RATIO_VOTOS_CANDIDATURA)
...
g_europeas_mun <- ggplot(data = df_plot,

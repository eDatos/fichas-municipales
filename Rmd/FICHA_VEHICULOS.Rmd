...
df_vehiculos_Canarias <- df_tipos %>%
...
df_vehiculos_mun <- df_vehiculos_mun %>%
  mutate(porcentaje = df_vehiculos_mun$valor / Num_Vehiculos * 100,
         valor_Canarias = df_vehiculos_Canarias$valor,
         porcentaje_Canarias = df_vehiculos_Canarias$valor / Num_vehiculos_Canarias * 100)
...
df_combustibles_mun <- df_tipos %>%
  filter(mun == municipio_actual$municipio & periodo == mes_actual$code & combustible != "TOTAL" & veh == "TOTAL" & substring(veh, 0,1) != " ") %>% select(!mun)
df_combustibles_Canarias <- df_tipos %>%
  filter(mun == "CANARIAS" & periodo == mes_actual$code & combustible != "TOTAL" & veh == "TOTAL" & substring(veh, 0,1) != " ") %>% 
  select(!mun)
df_combustibles_mun <- df_combustibles_mun %>%
  mutate(porcentaje = df_combustibles_mun$valor / Num_Vehiculos * 100,
         valor_Canarias = df_combustibles_Canarias$valor,
         porcentaje_Canarias = df_combustibles_Canarias$valor / Num_vehiculos_Canarias * 100)

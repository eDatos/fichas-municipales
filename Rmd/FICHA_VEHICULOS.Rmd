...
porc_otros <- ifelse(resto == 0, 0, 100 - porc_gasolina - porc_diesel - porc_electrico)
...
porc_otros_l <- ifelse(resto == 0, 0, round(100 - porc_gasolina_l - porc_diesel_l - porc_electrico_l, 1)) %>% as.numeric
```
...
resto_ <- sum(df_combustibles_mun %>%
               filter(combustible %notin% c("Gasolina", "Diésel", "Eléctrico", as.character(tipo_1), as.character(tipo_2), as.character(tipo_3), as.character(tipo_4))) %>%
               select(valor))
...
porc_o <- ifelse(resto_ == 0, 0, 100 - porc_1 - porc_2 - porc_3 - porc_4)
...
porc_o_l <- ifelse(resto_ == 0, 0, round(porc_otros_l - porc_1_l - porc_2_l - porc_3_l - porc_4_l, 1)) %>% as.numeric
```
...
if(Variacion_Porcentual == "NA"){
  cat(paste0(
    "<div class='column-3-ind'>", get_indicator2(paste0('Turismos<br/></p><p class="label" style="color: #999;">', paste0(Percent_Tur_Parque, '%')), Num_Tur_Parque, './img/coche.png'), "</div><div class='column-3-ind'>", get_indicator2("Gasolina<br/>+ diesel", paste0(format(porc_gasolina_diesel_l, big.mark = ".", decimal.mark = ",", nsmall = 1), '%'), "./img/gasolina.png"), "</div><div class='column-3-ind'>", get_indicator2("Eléctricos", paste0(format(porc_electrico_l, big.mark = ".", decimal.mark = ",", nsmall = 1), '%'), "./img/electrico.png"), "</div>"))
}else{
  cat(paste0(
    "<div class='column-4'>", get_indicator2('Tasa de<br/>variación', paste0(Variacion_Porcentual, '%'), Imagen_Variacion), "</div><div class='column-4'>", get_indicator2(paste0('Turismos<br/></p><p class="label" style="color: #999;">', paste0(Percent_Tur_Parque, '%')), Num_Tur_Parque, './img/coche.png'), "</div><div class='column-4'>", get_indicator2("Gasolina<br/>+ diesel", paste0(format(porc_gasolina_diesel_l, big.mark = ".", decimal.mark = ",", nsmall = 1), '%'), "./img/gasolina.png"), "</div><div class='column-4'>", get_indicator2("Eléctricos", paste0(format(porc_electrico_l, big.mark = ".", decimal.mark = ",", nsmall = 1), '%'), "./img/electrico.png"), "</div>"))
}
```
</div>
...
altura_resto <- 220
for(i in 1:4) {
  if(df_combustibles_otros[i,]$valor == 0){
...
if(porc_diesel_l == 0 & porc_electrico_l == 0 & porc_otros_l == 0){
  cat(paste0(
    "<div class='progressbar-legend'><div class='progress-bar-blue-0'></div><div class='sp-content'>Gasolina (", format(porc_gasolina_l, big.mark = '.', decimal.mark = ',', nsmall = 1), "%)"))
}else if(porc_electrico_l == 0 & porc_otros_l == 0){
  cat(paste0(
...
```{r, results='asis'}
if(porc_2 == 0 & porc_3 == 0 & porc_4 == 0 & porc_o == 0){
  cat(paste0(
    "<div class='progress-bar-blue-0' style='width:", porc_1, "%'>"))
}else if(porc_3 == 0 & porc_4 == 0 & porc_o == 0){
  cat(paste0(
    "<div class='progress-bar-blue1' style='width:", porc_1, "%'></div><div class='progress-bar-blue6' style='width:", porc_2, "%'>"))
}else if(porc_4 == 0 & porc_o == 0){
  cat(paste0(
...
if(porc_2 == 0 & porc_3 == 0 & porc_4 == 0 & porc_o == 0){
  cat(paste0(
    "<div class='progressbar-legend'><div class='progress-bar-blue-0'></div><div class='sp-content'>", tipo_1, " (", format(porc_1_l, big.mark = '.', decimal.mark = ',', nsmall = 1), "%)"))
}else if(porc_3 == 0 & porc_4 == 0 & porc_o == 0){
  cat(paste0(

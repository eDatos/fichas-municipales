...
mes_or <- levels(ordered(viajeros_lugar[["metadata"]][["temporalCoverages"]][["item"]][["id"]]))
...
num_viajeros_espanoles_mun <- df_mun$valor[which(df_mun$lugar == "España")]
num_viajeros_mundosinEspana_mun <- df_mun$valor[which(df_mun$lugar == "Mundo (excluida España)")]

peso_espanola <- ((num_viajeros_espanoles_mun / num_viajeros_num * 100)) %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character
peso_extranjera <- ((num_viajeros_mundosinEspana_mun / num_viajeros_num * 100)) %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character
```
...
peso_Canarias_Esp <- ifelse(
  num_viajeros_espanoles_mun == 0,
  "-",
  paste0((num_viajeros_canarios_mun / num_viajeros_espanoles_mun * 100) %>% round(1) %>% format(big.mark = ".", decimal.mark = ",", nsmall = 1) %>% as.character, "%"))
...
df_links <- links %>%
  filter(Tematica == "alojamientos_turisticos") %>%
...
<div class="row" style="margin: 24px 0 20px;">
  <div class="column-2">`r get_indicator3('España', paste0(peso_espanola, "%"), './img/spain2.png')`</div>

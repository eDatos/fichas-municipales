...
convocatoria <- convocatoria %>% 
  transform(convocatoria_formatted = paste0(unlist(lapply(strsplit(convocatoria, " "), '[[', 2)), " ",
                                            unlist(lapply(strsplit(substring(tolower(convocatoria), 0,3), " "), '[[', 1))))
...
total_votos_validos <- sum(df_plot_$VOTOS_VALIDOS_CANDIDATURA)
df_plot <- df_plot_ %>% mutate(porcentaje = cumsum(df_plot_$VOTOS_VALIDOS_CANDIDATURA) / total_votos_validos * 100)
...
g_congreso_mun <- plot_ly(df_plot,
  domain = list(x = c(0, 1), y = c(0, 1)),
  # value = df_plot$VOTOS_VALIDOS_CANDIDATURA[2],
  ...
  gauge = list(
    ...
    steps = list(
      list(range = c(0, df_plot$porcentaje[1]), color = "#005980"),
      list(range = c(df_plot$porcentaje[1], df_plot$porcentaje[2]), color = "#008BD0"),
      list(range = c(df_plot$porcentaje[2], df_plot$porcentaje[3]), color = "#00A6DC"),
      list(range = c(df_plot$porcentaje[3], df_plot$porcentaje[4]), color = "#2CBCE2"),
      list(range = c(df_plot$porcentaje[4], df_plot$porcentaje[5]), color = "#8CD2EA")),
    threshold = list(
...
if(Variacion_Porcentual_Elec == "NA"){
  cat(paste0(
    "<div class='column-2'>", get_indicator2(paste0('Electores</br>de Canarias</p><p class="label" style="color: #999;">', Num_Electores_Canarias), './img/female.png'), "</div><div class='column-2'>", get_indicator2(paste0('Electores</br>del extranjero</p><p class="label" style="color: #999;">', Num_Electores_Extranjero), './img/male.png'), "</div>"))
}else{
  cat(paste0(
    "<div class='column-3'>", get_indicator2('Tasa de</br>variación', paste0(Variacion_Porcentual_Elec, '%'), Imagen_Variacion), "</div><div class='column-3'>", get_indicator2('Electores</br>de Canarias', Num_Electores_Canarias, './img/female.png'), "</div><div class='column-3'>", get_indicator2('Electores</br>del extranjero', Num_Electores_Extranjero, './img/male.png'), "</div>"))
}
```
</div>
